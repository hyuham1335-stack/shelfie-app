---
description: 요청 하나를 계획 → 교차검증 → 구현 → 게이트(01~04)까지 끌고 간다. PR 은 만들지 않는다.
---

> **이 커맨드는 페이즈 01~04(계획 · 교차검증 · 구현 · 게이트)까지만 돈다.**
> 05~08(코드리뷰 · PR · PR리뷰 · 보고서)은 아직 구현되지 않았다.
...
- 런 디렉터리 경로
- 그리고 이 문장:
  > 05~08 은 아직 구현되지 않았다. 이 런은 **PR 을 만들지 않았고 push 하지 않았다.**
  > 코드는 워킹트리에 있다.
