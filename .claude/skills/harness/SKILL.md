...
20,000자 파일이면 12 turn 어치이고, turn 하나가 접두부 전체를 다시 읽게 만든다. 이것이 첨부 판단의 출발점이다. **다만 하한이다** — 어림은 *파일 크기*를 세지만 세션이 실제로 끌어오는 것은 테스트 출력·`grep` 결과·빌드 로그까지다. 런 #8 `capture-guide`는 어림 16,334자에 실측 **30,644자**로 1.9배였다.
...
4. **상한에 걸려 고를 수밖에 없다면 큰 파일부터 넣는다.** 아끼는 turn 수가 크기에 비례하므로, 같은 60,000자를 작은 파일 여럿으로 채우는 것보다 큰 파일 하나를 넣는 것이 낫다. **쓸 파일이라고 뒤로 미루지 마라** — 첨부본이 낡는 것은 *한 번*의 재독을 만들 뿐이고, 안 읽어서 생기는 여러 turn보다 싸다
5. **읽지도 않을 파일은 넣지 마라.** 아끼는 turn이 0인데 모든 turn이 값을 치른다
