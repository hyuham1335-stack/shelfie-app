...
- **`reason` 4종의 의미는 서로 겹치지 않는다.** `unreadable`(판독 실패) / `no_match`(알라딘에 정말 없음) / `ambiguous`(후보 복수) / `lookup_failed`(알라딘을 조회하지 못함 — 5xx·타임아웃·대조 예산 소진·브레이커 개방). 조회 실패를 `no_match`로 내보내면 사용자에게 "원서·절판일 수 있어요"라는 **사실이 아닌 설명**을 하게 된다 (ADR-005).
- **한 사유가 기전 여럿을 담는 것은 겹침이 아니다.** `unreadable`은 기전 셋(판독 확신도 미달 · 조회 상한에 밀림 · 제목을 정규화하면 빈 문자열)을, `lookup_failed`는 둘(ItemSearch 실패 · ItemLookUp 실패)을 담는다. **사용자가 할 수 있는 일이 같아서** 한 사유로 두는 것이고, 그래서 화면 문구도 하나다. 기전의 구분은 화면이 아니라 **계측**에 있다 — 이벤트 로그가 계측 사유 7종으로 나눠 세며 그 어휘와 한계는 `/docs/TRD.md` 6.4 에 있다.
- **응답 스키마는 접힘 규칙과 무관하다.** 계측이 "같은 책인가"를 묻는 데 쓰는 병합 키는 서버 안에서만 돌고 응답 경계를 넘지 않는다 — 이 문서의 어느 필드도, `src/lib/schemas.ts`의 어느 스키마도 그 이름을 갖지 않는다. 접힘 규칙이 바뀌어도 **응답 계약은 한 필드도 안 움직인다.**
- **미확인 카드 수가 늘어나는 것을 막는 장치는 이미 있다.** 제목을 판독하지 못한 후보들이 서로 "같은 책"으로 접히지 않게 되면서 한 요청이 내는 미확인 카드 수가 늘 수 있는데, 위의 `unidentified` 100건 상한과 `unidentifiedOverflowCount`가 그것을 그대로 받는다. **새로 둘 상한도, 고칠 문장도 없다.** 목록을 접지 않는 것은 의도다 — 사용자는 자기가 올린 사진마다 왜 빠졌는지를 봐야 한다 (ADR-002).
...
**설명**: 미확인으로 분류된 책을 알라딘에서 다시 찾아 후보를 돌려준다. (US-002, TR-008)

**호출자는 이 엔드포인트를 세 가지 경우에 쓴다.** 요청 형태는 셋 다 같고 다른 것은 `query`를 무엇으로 채우느냐다.

| 경우 | `query` | 왜 |
|------|---------|-----|
| 사용자가 제목을 고쳐 보냈다 | 사용자가 입력한 문자열 | 원래 용법 (US-002) |
| `ambiguous` 후보를 사용자가 골랐다 | 고른 후보의 제목 | **화면의 후보에는 `proof`가 없다.** 승격하려면 서명이 붙은 응답을 서버에서 다시 받아야 한다 (ADR-006). 응답에 고른 책의 `isbn13`이 없으면 승격하지 않고 재검색 화면에 남긴다 — 1번 후보로 때우지 않는다 |
| `lookup_failed` 책을 다시 시도한다 | 그 책의 `rawText` 그대로 | **고칠 제목이 없다.** 알라딘 조회가 5xx·타임아웃으로 실패한 것이지 검색 결과가 없는 것이 아니므로(ADR-005), 올바른 재시도는 **같은 질의를 다시 보내는 것**이다. 이 책 한 권만 다시 묻는다 — 사진 전체 재분석을 부르면 알라딘 한 건의 실패에 모델 비용 전체를 다시 낸다 |
**인증**: 불필요
...
- 이 응답의 책에는 `claudeNote`가 없다. 사용자가 후보를 확정한 뒤 클라이언트가 목록에 합류시키며, 한줄평은 비워 둔다. **없는 한줄평을 빈 문자열로 채워 생성 텍스트 블록을 그리지 않는다** (ADR-002).
- 서버는 세 경우를 구분하지 않는다. `query`가 사용자가 고친 제목인지 `rawText` 원문인지 아는 것은 클라이언트뿐이고, 서버가 그것을 알아야 할 이유가 없다 — **알 필요 없는 것을 계약에 넣으면 그만큼 위조할 것이 늘어난다.**
...
- **강행하기로 한 요청은 그 사실을 모델 호출에도 싣는다** (ADR-008). 서버가 `irrelevantStreak >= 2`로 판정을 무시하기로 했다면, **첫 호출부터** 추천 프롬프트가 "단서가 약해도 목록 안에서 최선을 고른다"로 바뀌어야 한다. 라우트에서만 422를 걷어내면 프롬프트는 여전히 "단서가 없으면 `recommendations`를 빈 배열로 둔다"고 지시하므로 **강행의 결과가 `200` + 빈 배열이 된다** — 화면에는 "그대로 골라 드릴게요"라고 적어 놓고 아무것도 주지 않는 상태다. **강행은 재호출이 아니라 첫 호출의 옵션이다**(모델 호출 횟수를 늘리지 않는다). 교정 재요청(위)이 일어나도 강행 여부는 유지된다. 강행 중에도 `relevant` 값 자체는 모델이 준 그대로 응답에 싣는다 — 서버가 무시하기로 한 것은 **그 값의 효력**이지 값이 아니다.
- `mood`는 사용자가 쓴 자유 텍스트이므로 **데이터로만** 다룬다. 시스템 프롬프트에 이어 붙이지 않고 사용자 메시지 안의 구분된 블록에 넣는다 (`/docs/TRD.md` 6.5 프롬프트 인젝션). 강행 지시문은 **우리가 쓴 지시문 쪽**에 넣고 `<mood>` 블록 밖에 둔다. 출력은 어차피 `isbn13` 화이트리스트로 걸러진다.
...
| 429 | 레이트 리밋 초과 — **`/api/analyze`에서만 발생한다** (TR-013 · ADR-012) | `Retry-After` 헤더를 **정수 초 문자열**로 함께 반환한다. 값은 남은 윈도 시간의 올림이고 **최소 1, 최대 한 윈도(60)**다. 본문은 다른 에러와 같은 형태이고 **`Retry-After` 값을 본문에 다시 싣지 않는다** — 두 곳에 두면 한쪽만 고쳐진다. 클라이언트는 지수 백오프 |
...
| `RATE_LIMITED` | 429 | 레이트 리밋 초과. **`/api/analyze` 한 라우트에만 배선돼 있다** — 나머지 넷은 이 코드를 내지 않는다 (TR-013 · ADR-012) |
