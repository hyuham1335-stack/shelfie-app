...
- **`lib/`의 순수 함수 규칙에 대한 예외는 `lib/rate-limit.ts` 하나다** (ADR-012). 이 모듈은 외부를 호출하지 않지만 **상태를 든다** — `globalThis`에 건 카운터 맵과 그 세대의 `windowStart`다. `resetRateLimit()`를 export해야 테스트가 상태를 되돌릴 수 있다는 것 자체가 순수하지 않다는 증거다. **서버 전용이고 클라이언트 컴포넌트가 import하지 않는다.** 위 캔버스 둘이 브라우저 API 때문에 예외인 것과 달리 이쪽은 **수명 때문**이라, 예외의 종류가 다르다는 것을 이름으로 적어 둔다.
