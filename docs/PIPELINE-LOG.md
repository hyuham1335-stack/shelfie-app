...
| 테스트 | `test_pipeline.py` 388 · `test_execute.py` 176 · `test_harness.py` 70 | **634건** (파일럿 앱은 1,347건) |
...
- **다음 작업은 M24·M25 다 — 런을 닫는 경로가 없다.** P2 가 01~07 을 완주하고 08 에서 막혔다. `record --phase 08` 이 미구현(exit 2)이고, `advance --phase 08` 은 04 게이트 지문과 대조하는데 **06 이 PR diff 를 위해 요구한 커밋이 그 지문을 반드시 낡게 만든다**(지문 = `HEAD + 소유 범위 미커밋 파일 해시`, `state.py:407`). 전체 게이트 재실행은 페이즈를 05 로 되돌리므로 쓸 수 없다. **`run_status` 가 영원히 `active` 로 남고, 다음 런의 `distinct_runs` 집계와 `audit_run` 주기가 그 위에 선다.**

    **P2 가 연 결함은 M24~M30 일곱이고, 07 의 내장 리뷰가 하네스 코드에서 7건(G-1~G-7)을 더 냈다.** 후자 중 G-4(`review05.status` 가 `failed` 가 될 수 없어 §E1 가드가 무력하다)가 우선순위가 높다. 상세는 [PILOT-LOG](harness/PILOT-LOG.md) 의 "파이프라인 런 P2".
...
실행기 7,424줄 · 파이프라인 테스트 388건 (전체 파이썬 테스트 634건) · 앱 테스트 1,347건
