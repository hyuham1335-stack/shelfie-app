...
| 층 | 실물 | 규모 |
|---|---|---|
| 실행기 | `scripts/pipeline/{cli,state,verdict,contract,gate,attribution,adapters,ledger,precheck,review,trace_contract,mask,pr,promote,review07,report}.py` — 16개 모듈 | **10,598줄** · Python 3.8 stdlib만 (서드파티 0) |
| 계약 계층 | `scripts/harness.py` — `doctor` · `calibrate` · `init` · glob 엔진 · 파일 목록 | 1,318줄 |
| 런타임 | `scripts/runtime.py` — 트랜스크립트 읽기 · 출력 인코딩. **코어가 실행기를 안 문다** ([[ADR-H037]]) | 310줄 |
...
| 테스트 | `test_pipeline.py` 712 · `test_execute.py` 185 · `test_harness.py` 79 · `test_runtime.py` 29 | **1,005건** (파일럿 앱은 **1,459건** · 48 suites — P8 이 +35) |
...
| 후속 (P8 런) | **`RATE_LIMITED` 가 이 리포에서 어휘로만 존재했다.** 스키마(`lib/schemas.ts:300`) · 서버 문구(`route.ts:86`) · 화면 문구(`ErrorBanner.tsx:40`) · 계약(`API_SPEC.md:335`) **네 층이 그 코드를 선언하는데 `src/` 전체에서 반환하는 자리가 0건**이었다 — 계약이 약속한 429 를 서버가 낼 수 없었고 화면은 **절대 오지 않는 코드의 문구**를 들고 있었다. 막고 있던 것은 기술이 아니라 한 문장의 오독이다: TRD 6.4 의 「인스턴스 간 메모리 공유 불가」는 **「완전한 리밋이 불가능하다」이지 「아무것도 못 한다」가 아니었다.** 조용히 통과할 뻔한 것은 **계약과 구현이 어긋난 채 초록불인 상태** 그 자체다 | 인스턴스별 in-memory **고정 윈도**를 골랐고 **못 막는 것 일곱을 ADR-012 와 TRD 양쪽에 값으로 적었다.** 01 에서 설계가 **네 번 뒤집혔다**(축출 → 열어 실패 → 넘침 버킷 → 고정 윈도) — 앞의 셋이 「차단된 공격자가 자기 카운터를 밀어낸다」·「상한이 차는 순간 방어가 꺼진다」·「신규 사용자를 막는 도구가 된다」로 깨졌고, 넷째에서 **정책이 아니라 포화가 값싸게 유지되는 구조 자체**를 바꿨다. 07 이 `escaped_05` **1건**을 냈다 — 429 가 FR-010 재시도 예산을 태워 **아직 막혀 있는 사용자가 재시도 버튼을 잃는** 회귀이고, **05 의 라우팅은 「바뀐 파일」 기준이라 구조적으로 이 종류를 못 본다**(변경되지 않은 `page.tsx` 와 새 429 의 상호작용). 그 자리에서 고쳤다. **이 런이 하네스 결함 둘(M56·M57)을 열었다** | `a372a78`·`6d99ca5`·`17deb6d` ([PR #11](https://github.com/hyuham1335-stack/shelfie-app/pull/11) · 앱 1424 → **1459** · 파이썬 829 → 837) |
| 후속 (하네스 결함 일곱 · 세션 C) | **P7·P8 이 연 일곱(M51~M57)이 열려 있었고, 그중 둘이 「하네스가 자기 기록을 지운다」 부류였다.** M53 은 델타 라운드 제출이 `need_more_context` 를 덮어써 **리뷰어가 "확인 못 했다"고 말한 것이 조용히 증발**했고(P7 에서 5건 → 0), M52 는 델타 단일 리뷰어 라운드가 **다른 리뷰어의 열린 지적을 회계에서 빼** PR 본문의 「미해결 Minor」가 줄었다. M56 은 라운드 예산 추가 지급이 다음 `counter_inc` 한 번에 지워져 **P8 에서 세 번 오탐 에스컬레이션**을 냈고, M57 은 `out_of_contract` 가 계약의 「데이터 형태」 절을 안 읽어 **P8 의 `contract-trace` 지적 6/6 이 구조적 오탐**이었다 | 일곱을 **결함별로 갈라** 닫았다. M56 은 `max` 를 「선언값 + `grants` 합」의 **파생값**으로 만들어 **저장·반환 두 자리**를 함께 고쳤다. M57 은 `config.contract.sections` 에 `data_shapes` 를 넣어 `symbols()` 가 그 절을 읽게 했다 — **템플릿을 안 건드리는 쪽**을 골랐다(반대안은 유닛 개수를 부풀려 프로파일 판정과 스코프 선택을 오염시킨다). M51 만 **코드가 옳아 문서를 고쳤다** — `quote` 의 대조 대상이 「diff 원문」이 아니라 「리뷰어 자신의 `.raw.md`」라고 스킬 다섯이 적게 했다. **ADR 을 새로 쓰지 않았다** — 다섯 다 새 결정이 아니라 정본을 구현·파생이 따라가게 한 것이다. **M56 의 기존 지급 테스트 다섯이 왜 이 결함을 놓쳤는지도 적었다**: 전부 `counter_grant` 직후만 봤고 그 뒤에 `counter_inc` 를 한 번 더 돌리는 단언이 하나도 없었다 — ADR-H025 의 「되돌려도 초록」 함정의 실례다 | `bfd8a7b`·`72bf339`·`5354067`·`e94e2df`·`5b84754`·`f96824f`·`da1cda6` (PR [#12](https://github.com/hyuham1335-stack/shelfie-app/pull/12)~[#21](https://github.com/hyuham1335-stack/shelfie-app/pull/21) · 파이썬 837 → **931**) |
| 후속 (자기 계측 · C2) | **여섯 런(P2~P8)의 보고서가 전부 *"소요 시간은 미측정이다"* 를 적었다.** `team-spec` 의 08 결정론 칸은 페이즈별 소요를 **이미 요구**하고 있었는데 구현이 없었다 — C3·C4 가 뒤이어 만난 「선언은 있는데 집계가 그걸 안 읽는다」와 같은 모양이다. 비용도 같았다: 절감 A/B 는 **앞선 순차 실행기**에서 잰 값이고 8페이즈 자신은 한 번도 재인 적이 없었다. 조용히 통과할 뻔한 것은 **자기 비용을 모르는 파이프라인이 「값을 낸다」고 주장하는 것** | 소요는 `events.jsonl` 의 **구간 분할**(인접한 두 `ts` 의 차를 그때 활성인 페이즈에 더한다), 비용은 `cost-state` 다. **`phase_enter`→`phase_pass` 짝을 안 쓴 이유를 실측으로 적었다** — P8 에서 02 의 Critical 이 01 로 되돌렸을 때 되돌아간 01 에 진입 이벤트가 없어 짝은 3시간 26분을 02 로 적고 합계가 벽시계의 **1.08배**가 되며, `08-report` 는 진입 이벤트가 아예 없어 **영영 못 잰다.** 구간 분할은 **`Σ 페이즈 소요 == 런 벽시계`** 라는 기계 검산 불변식이 10개 런 전부에서 정확히 성립했다. **첫 숫자가 파이프라인이 자기에 대해 낸 첫 답이다** — P8 은 벽시계 7시간 48분 중 **4시간 42분(60.2%)을 사람 기다림**에 썼고 그것이 다섯 건이었다. **비용은 런 보고서에 안 넣는다** — 08 이 그 세션 안에서 돌아 `cost-state` 가 아직 없다. 사각을 맨 앞에 적었다: P8 비용 **$4.34** 는 **마지막 세션 하나**다(과소) | `a6feae0`·`eb2f846`·`eb6c420`·`56ca599`·`8b2ef59`·`9349e4a`·`4b998e2` ([PR #13](https://github.com/hyuham1335-stack/shelfie-app/pull/13) · 파이썬 843 → **876**) |
| 후속 (승격 시한 · C3) | **`promote` 후보가 여섯 런째 0인데 언제 판정할지가 어디에도 없었다.** 판정 약속은 `ledger.py`·`promote.py` 주석에 처음부터 *"첫 세 런의 원장이 이 값을 검사한다"* 로 있었고 `distinct_runs` 는 **이미 6** 이었다 — **약속한 검사 시점을 이미 지나쳤는데 아무도 모르고 있었다.** 조용히 통과할 뻔한 것은 **「임계는 미검증 상속값이다」라는 표기가 영원히 유효한 변명이 되는 것** | `ledger.PROMOTION_VERDICT_AT_RUNS = 9` 와 `verdict_deadline()` 을 두고 보고서·`promote --scan` 이 찍게 했다. **게이트가 아니라 표시다** — 시한이 지나도 종료 코드도 등급도 안 바뀐다. **판정 전에 갈림 넷을 미리 적었다**([[ADR-H033]]) — 결과를 보고 해석을 고르지 않기 위해서다. 출발점도 숫자로 박았다: `candidates` 0 · `held` 0 · `distinct_runs` 6 · 최대 버킷 누적 2 · `NAMING` **86관측/84종 제목** · `rules_changelog.md` 데이터 행 0. **시한 자체가 미검증 상속값의 성질을 물려받는다는 것도 적었다** — 9 는 재서 나온 값이 아니고, 다만 **자기가 언제 검사받는지를 스스로 적고 있다** | `67cdfe6`·`251a412`·`a4bb70c`·`eed7ef6`·`1ff196e` ([PR #14](https://github.com/hyuham1335-stack/shelfie-app/pull/14) · 파이썬 876 → **884**) |
| 후속 (승격 축 · C4) | **시한만 걸어 놓고 3런을 더 돌리면 버킷만 늘고 후보는 여전히 0이다.** C3 을 닫으며 원장 158관측을 생산자별로 가르니 **`contract-trace` 가 93건(59%)**이고 그 제목은 **이미 템플릿**이었다 — `"계약에 없는 public 심볼 %s 가 생겼다"` 의 `%s` 만 다르다. `NAMING` **86관측이 84버킷**이고 84가 전부 `out_of_contract` **하나**다. [[ADR-H026]] 이 *"파일 경로를 키에 넣지 않는다"* 고 적었는데 **심볼 이름이 제목을 통해 키에 들어가** 같은 실수가 한 층 아래에서 반복됐다. 그리고 **통제 어휘는 이미 생산자 안에 있었다** — `trace_contract.CATEGORY` 가 검사 코드 다섯을 선언하고 `_finding` 이 그것을 싣는데 **집계가 그걸 안 읽었을 뿐**이다 | **`finding_key` 를 안 바꾸고 축을 하나 더 뒀다.** `rule_key` 는 `category`·`target_role`·`rule_slug` 의 sha1 이고 **슬러그가 없으면 `finding_key` 로 낙하**한다 — 기존 168줄에 슬러그가 없으므로 **과거 집계가 한 비트도 안 바뀐다.** 키를 바꿨다면 조용히 틀렸을 소비자 셋(`review.merge` 2인 합치 · `review07.escaped_05` · 05 단조성)을 **한 줄도 안 건드렸다.** 신뢰 경계는 `source == "contract-trace"` 하나로 좁혔다. **소급 부여는 안 했다** — 했다면 지금 당장 후보 1건이 나오고([[ADR-H026]] 이유 ②가 경고한 모양), 옛 줄에는 `code` 가 없어 기술적으로도 불가능하다. **판정 시점이 P9~P11 로 옮겨 갔다** — ADR-H033 의 `distinct_runs` 9 는 「축이 안정된 표본」을 전제하고 축을 바꾼 직후의 런은 그 전제를 만족하지 않는다 | `ed85d23`·`e773adf`·`3e03517` ([PR #15](https://github.com/hyuham1335-stack/shelfie-app/pull/15) · 파이썬 884 → **899**) |
| 후속 (작업 원칙 넷) | **넷 다 이 리포가 이미 하고 있는데 글로는 없던 규율이었다.** Surgical Changes 는 P4 가 런 도중 어댑터를 안 고친 것이고, Simplicity First 는 ADR-H029 가 예산 값을 안 가른 것이고, Goal-Driven 은 ADR-H033 이 판정 갈림을 미리 표로 적은 것이다. **하고 있는데 안 적히면 다음 사람이 물려받지 못한다** — 그것이 조용히 통과할 뻔한 것이다 | `CLAUDE.md` 에 넷을 세웠다. **스킬이 아니라 `CLAUDE.md` 인 이유를 적었다** — 이 리포의 스킬은 `when` glob 으로 파이프라인이 결정론적으로 부르는 것이고, 항상 켜져 있어야 하는 원칙에는 걸 glob 도 페이즈도 없다. **부를지 말지를 고르는 원칙은 원칙이 아니다.** 4번(TDD)은 **기존 줄에 흡수**시켰다 — 나란히 두면 같은 규율이 한 파일 두 자리에 생기고 그 파일 자신이 금하는 모양이 된다. 1번은 *"요청이 모호하거나"* 에서 **"선택에 따라 결과가 달라질 때"** 로 좁혔다 — ROADMAP 이 이미 반대편을 경고한다(*"매번 사람을 세우는 검사는 검사가 아니라 소음이 되어 간다"*). **비용을 절 안에 적었다**: 6,496 → 7,790 바이트(+19.9%)이고 이 파일은 매 turn 접두부로 다시 읽히므로 **이 절을 넷 넘게 늘리지 않는다고 파일 자신이 말한다** | `ec3b66d` ([PR #16](https://github.com/hyuham1335-stack/shelfie-app/pull/16) · 코드 변경 0줄) |
| 후속 (통제 어휘 · C5) | **C4 가 축을 갈랐는데 그 축을 쓰는 생산자가 하나뿐이었다.** 원장 168줄 중 리뷰어 54 + code-review 21 = **75줄이 폴백 경로**에 남았고 집계가 그것을 그대로 찍었다 — `DOC_CODE_DRIFT` **18관측/18버킷** · `TEST_MISSING_FAILURE_PATH` **18/18**. **`count == distinct_keys` 가 "반복된 적이 없다"는 뜻이 아니었다** — 제목을 읽으니 `nothing_locked` 모양 8관측이 **4런을 가로지르는데** 제목이 매번 달랐다. C4 가 `out_of_contract` 에서 본 것과 같은 모양이고, 통제 어휘가 코드 안에 없다는 점만 달랐다 | `taxonomy.json` 의 카테고리 객체에 `slugs` 를 두고 **`validate_taxonomy` 에 검증을 신설**했다(형태는 `_SLUG_SHAPE` **그 객체**를 재사용). 신뢰 경계를 셋으로 넓히고(`contract-trace`·`reviewer`·`code-review`), **어휘 대조는 제출자 층**이 한다 — 위반한 그 제출자가 예산·강등을 탄다(M46 2층 규율). **어휘의 출처와 거처를 갈랐다**: 출처는 리뷰어 스킬 5종의 「볼 것」 표이고 거처는 `taxonomy.json` 이지 **스킬 파일이 아니다** — `DOC_CODE_DRIFT` 18건을 낸 것은 **arch 7 · data 5 · sec 3 이고 `docs` 는 0** 이라, **카테고리가 스킬을 가로지르므로** 어휘를 스킬 파일에 두면 그것을 안 읽는 리뷰어가 그 카테고리를 낸다. **보증이 약해졌고 그것을 적었다** — `CATEGORY` 는 코드에서 닫혀 생산자가 다른 값을 *낼 수 없고*, `slugs` 는 데이터에서 닫혀 생산자가 무엇이든 내고 *exit 8 을 받는다* | `51d4e6f`·`9b9a5e7`·`4da7572` ([PR #22](https://github.com/hyuham1335-stack/shelfie-app/pull/22) · 파이썬 931 → **967**) |
| 후속 (M58·M59 · C7) | **`SessionEnd` 훅이 여섯 런 동안 남의 런을 자기 줄에 달고 있었다**(M59). `session_log._latest_run` 이 **가장 최근 런 디렉터리를 무조건** 집어, 런이 닫힌 뒤 시작한 세션에도 그 `run_id` 를 달았다 — 원장이 자기 근거로 쓰는 `tests.app`·`run.grade` 가 **그 세션의 사실이 아닌** 것이 된다. **이 문서의 §5 가 원장을 근거로 삼으므로 틀린 귀속이 그대로 승격 행의 숫자가 된다.** 함께 M58 — `counter_grant` 의 `setdefault` 초기값이 노드가 없을 때 **상한을 2배**로 만든다(직접 호출로 재현) | 귀속을 **구간 겹침**으로 판정하고 **안 겹친 사실을 적는다** — `run.basis` 가 `latest_only` 면 「런은 있었지만 이 세션이 만진 적이 없다」는 뜻이고 `tests.source` 도 그때는 `calibration` 이다. **훅을 고치는 대신 읽는 시점에 우회했다** — 훅을 고치면 앞으로 쓸 줄만 나아지지만 읽는 시점 판정은 **기존 46줄에 소급 적용**된다. `basis` 칸이 아예 없는 줄(2026-09-10 이전 전부)은 **판정 불가**로 남기고 그렇게 표기했다 | `cbf8899`·`4febf81`·`884a7b6`·`a860d09`·`0d223ad` ([PR #23](https://github.com/hyuham1335-stack/shelfie-app/pull/23) · 파이썬 967 → **975**) |
| 후속 (템플릿 추출 · D~G) | **하네스가 이 리포 안에서만 존재해 「스택이 안 박혔다」는 주장이 검증될 자리가 없었다.** [[ADR-H003]] 이 *"빈 템플릿 리포에서는 `doctor` 가 검사할 실물이 없어 통과가 아무것도 의미하지 않는다"* 며 추출을 미뤄 뒀고, 그 조건이 P1~P8 로 채워졌다. 추출을 시작하니 **코어가 실행기를 물고 있었다** — `state.py`·`cli.py`·`session_log.py` 가 `execute.py` 를 import 하고 `state.py` 는 **모듈 최상위**라, 그대로 추출하면 `test_pipeline.py` **712건이 전멸**한다. 그리고 픽스처가 실물 config 를 복사해 **어댑터를 교체하자 290건이 빨개졌다** — 픽스처가 Next.js 모양이면서 자기 스택을 선언하지 않았기 때문이다 | 네 세션으로 갈랐다. **D** — 트랜스크립트 읽기와 출력 인코딩을 `scripts/runtime.py` 로 내리고 `CoreDoesNotImportTheExecutorTest` 로 잠갔다(커밋 362 → 187 · 파일 69 · **수집 에러 0**). **E** — 코어 스크럽 15군데 · `harness/adapters/self-python.json` 신설(`nextjs-ts` 는 씻어서 예시로 남김). **F** — 문서를 성질에 따라 다르게 처리했다: `ROADMAP` 재작성(799→198줄) · `PILOT-LOG` **빈 골격**(2,927→81줄) · `DECISIONS` 는 **씻어서 통째 보존**(ADR 한 줄도 안 지우고 숫자 한 자도 안 고쳤다). **G** — 게이트 ①~④ 통과 후 push. **마지막 빨간불은 `remote add` 가 아니라 push 가 풀었다**(`_check_remote` 가 `refs/remotes/origin/main` 을 요구한다). **게이트 ⑤(01~04 실물 완주)는 안 돌렸고 「미실행 = 통과 아님」으로 채점표에 빈칸을 남겼다** — 사람이 「기계 검사만」을 골랐고, 그 사실을 [[ADR-H040]] 결정 2 와 ROADMAP 39번에 적었다. **추출본 실측: 커밋 190 · 파일 70 · `pytest` 821 통과 · `doctor` exit 0** | `f24aaa2`·`ae62e58`·`05d09c9`·`9acb8f6`·`2e86871`·`11645b9`·`bd031d3` (PR [#24](https://github.com/hyuham1335-stack/shelfie-app/pull/24)~[#27](https://github.com/hyuham1335-stack/shelfie-app/pull/27) · 파이썬 975 → **1,005**) |
...
**아홉 행을 한 번에 올렸고, 그 자체가 이 문서의 결함이다.** 마지막 승격은 2026-09-08 이고 그 뒤 **세션 19줄**이 원장에 쌓이는 동안 §5 가 한 줄도 안 자랐다. `/log` 는 사람이 부를 때만 도는 설계이므로(`b9a8274` 행이 그 이유를 적는다) 안 부르면 안 도는 것이 정상 동작이지만, **한 번에 아홉을 올리면 각 행이 "그때 무엇을 알았는가"가 아니라 "지금 돌아보니 무엇이었는가"가 된다.** 같은 성질의 빈칸이 `PILOT-LOG.md` 에도 있었고 그쪽은 ROADMAP 항목 38 로 등록돼 있었다.

**행별 근거를 가른다 — 여섯은 원장이고 셋은 git 이다.**

| 행 | 근거 | 왜 |
|---|---|---|
| P8 런 · M51~M57 · C2 · C3 · C5 · C7 · D~G | **원장 + 커밋 본문** | 세션 줄이 그 커밋들을 담고 있다 |
| **승격 축 · C4** | **git 만** | `e773adf`·`ed85d23`·`3e03517` 을 담은 **세션 줄이 원장에 없다** |
| **작업 원칙 넷** | **git 만** | `ec3b66d` 을 담은 세션 줄이 원장에 없다 |
| M54·M55 (위 「일곱」 행에 포함) | **git 만** | `385d33c`~`1dec248` 을 담은 세션 줄이 원장에 없다 |

**원장에 구멍이 셋 있다.** `56ef8be..HEAD` 의 non-merge 커밋을 원장의 `commits[].sha` 와 전수 대조하니 **13건이 어디에도 안 잡혔고**, 그중 열은 위 세 묶음이다(나머지 셋은 원장 줄을 만드는 `chore(ledger)` 커밋 자신과 이 승격을 쓰는 커밋이라 정상이다). 세 세션 다 `SessionEnd` 훅이 안 돈 것으로 보이나 **왜 안 돌았는지는 확인하지 못했다** — 원장에 그 사실을 남기는 자리가 없기 때문이고, 그래서 이 문단이 그 자리다. 훅이 안 돌면 **그 세션이 한 일이 §5 에서 통째로 사라진다.**

**`run.*` 를 안 믿은 줄이 있다.** M59([[ADR-H036]])가 고친 대로, `run.basis` 가 `latest_only` 인 줄은 「런은 있었지만 이 세션이 만진 적이 없다」는 뜻이다 — C7·D·E·F·G 다섯 세션 줄이 그 상태이고 `tests.app: 1403 (source: calibration)` 은 **그 세션들의 측정이 아니다.** 2026-09-10 이전 줄은 `basis` 칸이 아예 없어 **판정 불가**다. 그래서 위 행들의 앱 테스트 수는 원장이 아니라 **P8 게이트 영수증**에서 왔다 — `_workspace/runs/20260908-1720-dca1/04_gate_report.json` 의 `full` 스테이지가 `Test Files 48 passed · Tests 1459 passed` 를 실제로 찍었다. **하네스 세션 여덟은 `src/` 를 한 줄도 안 고쳤으므로 앱 1,459 는 그대로이고 다시 재지 않았다.**

**파이썬 건수는 테스트를 돌려 만든 값이 아니라 `def test_` 정적 계수다.** 각 커밋 시점의 `scripts/test_{pipeline,harness,execute,runtime}.py` 를 `git show` 로 꺼내 세었고, 사슬은 829(P7) → 837(P8 런) → 843(M56) → 876(C2) → 884(C3) → 899(C4) → 931(세션 C 나머지) → 967(C5) → 975(C7) → **1,005**(D · E·F·G 는 문서라 그대로)다.

**그 계수가 어긋남 하나를 드러냈다.** `docs/harness/ROADMAP.md` 의 C3 절은 *"파이썬 테스트 876 → 885"* 라 적는데 그 커밋(`eed7ef6`)의 본문은 *"876 -> 884"* 이고 **정적 계수도 884** 다. **1건 차이이고 이 문서에서는 884 를 쓴다.** `docs/harness/**` 는 `/log` 가 고치지 않으므로(프로젝트 규율) **고치지 않고 여기 적어만 둔다** — 이 리포의 어휘로는 `DOC_CODE_DRIFT` 다.

**§1 규모 표를 정적 계수로 다시 대조해 갱신했다** — 실행기 9,571 → **10,598줄**(모듈 16 그대로) · 계약 계층 1,295 → **1,318줄** · 파이썬 829 → **1,005건**(`test_pipeline` 712 · `test_execute` 185 · `test_harness` 79 · **`test_runtime` 29**) · 앱 1,424 → **1,459건 · 48 suites**. **`scripts/runtime.py` 는 새 행이다** — D 가 코어에서 실행기를 떼며 만든 자리이고([[ADR-H037]]) `test_runtime.py` 가 그것을 잠근다. §6.5 훅 표는 `.claude/settings.json` 과 대조해 `SessionEnd`·`Stop`·`PreToolUse` **셋 그대로**였다.

**묶음의 기준은 기존과 같다** — 「한 증분이 한 세대의 결함을 닫은 것」이면 한 행이고, 쪼개면 같은 판단이 §5 에서 여러 번 학습된다. 그래서 M51~M57 일곱이 한 행이고 D~G 넷이 한 행이다. **다만 C2~C5 는 갈랐다**: 넷이 서로를 낳긴 했어도(C3 이 C4 를 열고 C4 가 C5 를 열었다) **각각 다른 결함에 답한 독립 증분**이고 ADR 도 넷이다. **이 승격을 쓰는 커밋 자신과 원장 표시 커밋은 행으로 만들지 않는다.**
