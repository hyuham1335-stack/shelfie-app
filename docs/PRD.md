...
**`recommend_viewed`는 서버가 남긴다.** 렌더 시점이 아니라 응답 반환 직전이며, `duration_ms`와 토큰은 서버만 알기 때문이다. 양쪽이 보내면 **추천 수락률의 분모가 두 배로 부푼다** — `/api/events`의 허용 목록에 이 이름이 아직 남아 있으나 클라이언트는 보내지 않는다.
