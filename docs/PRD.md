...
| FR-012 | 조회 전 후보 상한 | 알라딘 조회 전에 판독 확신도 0.3 미만은 "못 읽음"으로 내리고, 남은 후보를 확신도 높은 순 **65건**까지만 조회한다. 미확인 목록은 100건까지만 보여준다. 상한이 없으면 판독 한 번의 이상 동작이 알라딘 일일 한도를 한 요청에 소진시킨다. **65는 임의의 수가 아니라 세션당 260회(TRD 10번)에서 유도된다** — 한 후보가 ItemSearch·ItemLookUp 두 단계를 각각 최대 2회씩 태우므로 `65 × 2 × 2 = 260`이다 |
| FR-013 | 알라딘 상품 링크 | 확인된 책과 추천 책 카드에서 알라딘 상품 페이지로 나가는 링크를 제공한다. URL은 **알라딘이 준 `aladinLink`를 그대로** 쓰고 ISBN으로 조립하지 않는다 — 조립한 URL은 우리가 만든 사실이다. 새 창으로 열고(`rel="noopener noreferrer"`), 링크가 없는 책에는 자리만 비운다. **이벤트를 만들지 않는다** (아래 "왜 이벤트를 만들지 않는가") |
| FR-014 | 추천 결과 이미지 저장 | 추천 결과를 PNG 한 장으로 내려받는다. 이미지는 **클라이언트에서만** 만들고 서버로 보내지 않으며 어디에도 저장하지 않는다(보관 기간 0은 여기서도 유지된다). 이미지 안에서도 **알라딘 사실과 Claude가 쓴 추천 이유를 시각적으로 가른다** — 화면에서 지키는 경계가 밖으로 나가는 그림에서 무너지면 그 그림이 사실로 유통된다 (ADR-002). 표지를 그리지 못하면 표지 없이 그리고 저장을 취소하지 않는다 |
| FR-015 | 촬영 가이드 | 업로드 화면에서 촬영 요령을 **펼쳐 볼 수 있는 시트**로 제공한다. 기본은 접힌 상태이고 촬영 팁 한 줄(항상 보이는 것)은 그대로 둔다. **카메라 프리뷰 위의 오버레이가 아니다** — 촬영이 `<input type="file" capture>`로 시스템 카메라를 여는 구조라 우리 페이지가 프리뷰 위에 그릴 수 없다 |
...
| **Could have (이번 릴리스에 넣는다)** | 추천 결과 이미지로 저장(FR-014), 알라딘 상품 링크(FR-013), 촬영 가이드 시트(FR-015) | 가치는 있으나 핵심 루프 밖이다 — Must·Should가 전부 구현된 뒤에 붙인다. **셋 다 새 외부 의존을 만들지 않는다**: 링크 URL은 이미 응답에 실려 오고(`aladinLink`), 이미지는 클라이언트 캔버스로 그리며(ADR-009), 가이드는 정적 콘텐츠다. "촬영 가이드 오버레이"라고 적었던 것을 **시트로 정정했다** — 오버레이는 이 앱의 촬영 경로에서 구현할 수 없다 (FR-015) |
| **Won't have (this release)** | 로그인·계정, 서재 누적 저장, 읽은 책 기록, 사용자 간 공유·피드, 전자책 연동 | 저장을 넣는 순간 데이터 레이어·인증·개인정보 처리방침이 함께 필요해져 MVP 범위가 두 배가 된다. 무상태로 가설부터 검증한다 (ADR-003). 다음 릴리스에서 저장이 필요해지면 Supabase로 간다 — 무엇을 쓸지는 이미 정해 두었고, 남은 질문은 시점뿐이다 (ADR-007) |
...
| 화면 | 목적 | 핵심 요소 | 관련 스토리 |
|------|------|-----------|-------------|
| 업로드 | 첫 행동까지의 마찰을 최소화 | 촬영·선택 버튼, 선택된 사진 썸네일(최대 5), 촬영 팁 한 줄, **접힌 촬영 가이드 시트(FR-015)**, 분석 시작 CTA | US-001 |
| 분석 결과 | 내 서재를 사실 기반으로 확인 | 확인된 책 카드 목록(표지·서지·평점·한줄평·**알라딘 링크(FR-013)**), 미확인 섹션(원문·사유·수정 입력), 추천으로 넘어가는 CTA | US-001, US-002 |
| 기분 입력 | 추천 조건을 한 줄로 받기 | 자유 텍스트 입력창, 예시 문장 3개, 비워 두고 진행하는 경로 | US-003, US-004 |
| 문답 | 기분을 모를 때 좁혀 주기 | 서재 기반 질문 2~3개와 선택지, 건너뛰기 | US-004 |
| 추천 결과 | 가치 획득 | 추천 3권 카드(표지·서지·추천 이유·**알라딘 링크(FR-013)**), "이거 읽을래요" 버튼, **이미지로 저장(FR-014)**, 다시 추천받기 | US-003 |
...
|------|---------------------|-----------------|
| 저장소·계정 없음 (무상태) | 등록 마찰 0이 이 제품의 핵심 차별점이고, 데이터 레이어를 넣으면 MVP 범위가 두 배가 된다 (ADR-003) | 사용자가 "찍은 서재를 다시 보고 싶다"고 요구하기 시작할 때. 그때 도입할 저장소는 Supabase로 확정돼 있다 (ADR-007) |
