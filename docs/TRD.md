...
**세션당 260회는 산문의 계산식이 아니라 코드가 잠그는 계약이다.** 그 값은 `lib/env.ts`의 두 상수에서 **유도된다** — 조회 전 후보 상한 `MAX_CANDIDATES_FOR_LOOKUP`(80)과 확인 상한 `MAX_IDENTIFIED_BOOKS`(50)이고, 각각 ItemSearch·ItemLookUp을 최대 2회씩 태우므로 `(80 + 50) × 2 = 260`이다. 상한 자체를 `lib/env.ts`에 이름 있는 상수로 두고, **두 상한에서 유도한 값이 그 상한을 넘지 않는지 회귀 테스트로 고정한다.** 누군가 80을 200으로 올리는 순간 알라딘 일일 한도(5,000회)가 세션 8회에 소진되는데, 지금은 그것을 막는 것이 이 표의 산문뿐이다 — **문서에만 있는 상한은 검증되지 않은 상한이다.** 이 게이트는 인스턴스 간 카운터가 아니라 **한 요청 안의 구조적 상한**이므로 무상태 전제(ADR-003)와 충돌하지 않는다. 일일 누적 카운트와 ISBN 캐시는 여전히 `[Scale]`이고 Supabase 도입 시점의 몫이다 (ADR-007).
