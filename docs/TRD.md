...
| TR-005 | FR-004, FR-005 | `lib/merge.ts` — ① 알라딘 조회 **전**에 `confidence < 0.3` 후보를 `unreadable`로 강등하고, 제목+저자 정규화 키로 사전 병합한 뒤 **`confidence` 내림차순 65건으로 자른다**(세션당 260회에서 유도된 값 — 10번) ② 조회 후 ISBN13 기준 중복 제거(최초 등장 사진 인덱스 유지) ③ 확인 50권·미확인 100건 상한 절단 | 같은 책이 5장에 모두 등장해도 결과 1권이고 **알라딘 호출도 1회**, 51권 입력 시 정확히 50권 + `overflowCount: 1`, 후보 300건을 넣어도 조회 대상은 65건을 넘지 않고 ItemSearch·ItemLookUp 이 같은 수를 받음 | 절단 순서는 **결정적**이어야 한다: `aladinRating` 내림차순 → null은 최하위 → 동점은 `photoIndex` 오름차순 → `isbn13` 오름차순. ISBN13이 없는 책은 애초에 확인으로 올라오지 않으므로(TR-004) 대체 키 병합 경로는 두지 않는다 | `npm test -- merge.test.ts` | [MVP] |
...
| TR-010 | US-003, FR-006, FR-009, FR-011 | `app/api/recommend/route.ts` — **① 각 책의 `proof` 서명을 검증해 통과분만 남기고(0권이면 400 `UNVERIFIED_BOOKS`) ②** 확인된 책 목록 + 기분 텍스트로 3권 추천 **③ 응답의 모든 `bookId`가 검증 통과 목록에 있는지 확인**, 위반 시 1회 재요청 후에도 실패하면 502 | 목록 밖 책 노출 0건, 위조된 책 노출 0건, 응답 p95 15초 이내 | 책 3권 미만이면 있는 수만큼. 무관한 기분 입력은 모델 구조화 출력의 `relevant: false`로 판정해 `IRRELEVANT_MOOD` 반환하되, **같은 세션에서 2회 연속 무관 판정이면 판정을 무시하고 추천을 진행**한다(오탐으로 사용자를 가두지 않는다). 이때 판정은 모델 호출 **앞에서** 내려 **첫 호출부터** 추천 프롬프트의 관련성 규칙을 대체한다 (ADR-008) — 라우트에서만 422를 걷어내면 모델은 규칙대로 빈 배열을 돌려주므로 강행이 `200` + 추천 0건으로 끝난다. 강행은 재호출이 아니라 첫 호출의 옵션이라 **호출 수를 늘리지 않는다**(6.1 표 유효). 재요청 시에는 동일 프롬프트를 반복하지 않고 **위반한 `bookId`를 명시하고 허용 목록을 다시 제시**한다 | `npm test -- recommend.route.test.ts` | [MVP] |
...
| TR-012 | PRD 7번 | `lib/analytics.ts` — 9종 이벤트를 구조화 JSON 한 줄로 표준 출력. `session_id`는 클라이언트 생성 UUID, PII·이미지·파일명·`rawText` 미기록. Claude를 호출하는 모든 이벤트에 `input_tokens`·`output_tokens`를 싣는다 | 9종 이벤트 전부 발생 지점에서 호출되고, 세션당 비용을 로그만으로 합산할 수 있음 | 로깅 실패가 요청 처리를 막지 않는다(try/catch로 삼킴) | `npm test -- analytics.test.ts` | [MVP] |
| TR-013 | 6.5 보안 | `lib/rate-limit.ts` — **`/api/analyze` 한 라우트에만** IP당 분당 20회 레이트 리밋, 초과 시 429 + `Retry-After`. 카운터는 **인스턴스별 in-memory 고정 윈도**이고 상태는 `globalThis`에 건다 (ADR-012) | 429 정상 반환, 정상 트래픽 오탐 0건 | **실효 상한은 `20 × 인스턴스 수`이고 그 수를 재지 않았다.** 콜드 스타트가 카운터를 지우고, 분산 공격자는 못 막는다 — **남용 완화이지 방어가 아니다.** 나머지 라우트 넯은 안 덮이며 아래 수용 부채에 있다. 인스턴스 간 공유가 필요해지면 Supabase (ADR-007) | `npm test -- rate-limit.test.ts` | [MVP] |
...
interface ExtractedCandidate {
  ...
  confidence: number;       // 0.0~1.0. 판독 자체에 대한 모델의 확신도.
                            ...
                            // 조회 전 후보 상한(65건)을 넘으면 이 값 내림차순으로 자른다 (TR-005)
...
- **조회하지 않았거나 하지 못한 책은 미확인 비율 가드레일의 분자에도 분모에도 넣지 않는다** — `lookup_failed`(알라딘 장애)와 조회 상한에 밀린 책 둘 다. 외부 장애를 프롬프트 품질 저하로 오독하면 엉뚱한 롤백을 하게 되고(ADR-005), 반대로 분모에만 남기면 알라딘 전면 장애 세션이 0%로 가장 좋은 성적을 낸다. 정확한 분자·분모와 한계는 아래 「미확인 비율 가드레일」을 본다.
- 로그 레벨 기준: `error`는 사람의 조치가 필요한 것만(5xx, 할루시네이션 검증 실패). 예상된 4xx와 미확인 강등은 `warn` 이하.
- 로그 보관: Vercel 기본 정책(30일).

#### 미확인 비율 가드레일 — 계측 어휘와 그 한계

**공식은 이것 하나다.** 다른 조합으로 나누면 안 된다.

```
미확인 비율 = raw_unidentified_guardrail_count ÷ raw_guardrail_denominator
```

`analyze_completed`가 싣는 값의 층위가 둘이라 **이름이 그것을 나른다.**

| 필드 | 무엇 | 기준 |
|------|------|------|
| `raw_guardrail_denominator` | 확인된 책 + 분자 대상 미확인 = **가드레일 분모** | 절단 전 · 접힘 후 — **확인 경계를 가로지른다** |
| `raw_unidentified_guardrail_count` | `low_confidence` + `blank_title` + `no_match` + `ambiguous` = **분자** | 절단 전 · 접힘 후 — **확인 경계를 가로지른다** |
| `raw_unidentified_by_measurement` | 계측 사유 일곱의 분해 | 절단 전 · 접힘 후 — **확인 경계를 가로지른다** |
| `raw_candidate_count` | 확인 + 미확인 전부. **관측용 총수이지 분모가 아니다** | 절단 전 · **접힘 전** |
| `identified_count` · `unidentified_count` · `unidentified_by_reason` · `overflow_count` | 기존 값 | **절단 후** |

**"접힘 후"의 뜻이 넓어졌다.** 예전에는 미확인 목록 **안에서만** 같은 `mergeKey`를 접었고, 지금은 확인된 책의 키까지 본다 — 같은 책의 흐릿한 판독본이 또렷한 판독본과 함께 확인으로 올라갔으면 미확인 쪽에서 세지 않는다. **접힘 범위는 일곱 칸이 같다**: 사유는 "같은 책인가"라는 질문에 답하지 않으므로 칸마다 다른 규칙을 두지 않는다. 그리고 **정규화 제목이 빈 항목은 서로 접힐 수 없다** — 빈 키는 "같은 책"의 증거가 아니라 증거의 부재다.

**응답 사유는 4종 그대로이고 계측 사유는 일곱이다.** 응답의 `unreadable` 한 칸이 기전 셋(`low_confidence`·`lookup_capped`·`blank_title`)을 담고, `lookup_failed` 한 칸이 둘(`search_failed`·`facts_failed`)을 담는다. 사용자가 할 수 있는 일이 같으면 같은 문장을 봐야 하므로 화면에서는 쪼개지 않고, 지표는 반대라 계측에서만 쪼갠다.

**조회하지 않았거나 하지 못한 책은 분자에도 분모에도 안 들어간다** — `lookup_capped`(우리가 건 상한에 밀림)·`search_failed`·`facts_failed`(그 단계에서 알라딘 응답을 확보하지 못함, **호출조차 못 한 경우 포함**)는 프롬프트 품질 판정의 모집단 밖이다. 이 셋을 분모에만 남기면 알라딘 전면 장애 세션이 0%로 **가장 좋은 성적**을 낸다.

**단 이 경계가 한 번 움직였다 — 정규화 제목이 빈 후보는 이제 조회 대상이 아니다.** 예전에는 그런 후보도 조회 대기열에 들어갔고 65칸 뒤로 밀리면 `lookup_capped`(모집단 밖)였다. 지금은 조회 **전에** 갈려 상한과 무관하게 항상 `blank_title`(모집단 안)이다. **분자 +1 · 분모 +1 이고 지표가 나빠지는 방향이며, 그것이 옳다** — 예전에는 읽히지 않은 책등이 지표에 잡히는지가 그 요청에 다른 후보가 몇 개 있었는지에 달려 있었다. 같은 사진, 같은 판독 실패인데 옆에 후보가 많으면 "우리 상한 탓"이 되고 적으면 "판독 탓"이 됐다. 판독 품질을 재겠다는 지표가 판독과 무관한 변수에 흔들린 것이고, 지금은 **읽혔는가 하나**만 그 분류를 정한다.

**그리고 빈 키를 안 접게 된 것이 `blank_title` 한 칸만 움직이는 것이 아니다.** `null` 키는 계측 사유를 보지 않고 접힘에서 빠지므로 **일곱 칸 전부**에서 풀린다. 실제로 가장 크게 움직이는 칸은 `blank_title`이 아니라 **`low_confidence`** 다 — 조회 전 축소가 확신도 하한을 **먼저** 가르기 때문에, 확신도가 0.3 미만이면서 제목도 빈 후보는 `blank_title`이 아니라 `low_confidence`로 분류되고 그 항목의 키가 `null`이다. 그런 판독본 20건이 옛 세계에서는 한 키로 접혀 **1**이었고 지금은 **20**이다. `low_confidence`는 분자에 드는 칸이므로 **분자와 분모가 함께 19 오른다.** 이것도 지표가 나빠지는 방향이고 같은 이유로 옳다 — 제목을 못 읽었으면 같은 책인지 물을 근거가 없고, 근거 없이 접는 쪽이 판독 품질을 좋게 보이게 만든다.

**그래서 `lookup_capped`가 두 경로로 줄어든다. 둘의 성질이 다르므로 갈라서 적는다.**

| 경로 | 무엇이 빠지나 | 되돌릴 수 있나 |
|------|--------------|---------------|
| 조회 전 분기 | 빈 제목 후보가 `blank_title`로 옮겨 간다 | **바구니 경계만** — `lookup_capped + blank_title`의 합이 옛 `lookup_capped`와 같은 자리를 본다. **그 합의 절대값은 되돌릴 수 없다**(아래 ⓒ) |
| 확인 경계를 넘는 접힘 | 확인된 책과 키가 겹치는 **저확신 판독본**이 `low_confidence`에서 빠지고 **어느 칸에도 안 남는다** | **안 된다** — 분해표에서 완전히 사라지므로 어떤 합으로도 복원되지 않는다 |

**둘째 경로가 닿는 칸은 `low_confidence` 하나다.** 조회 전 축소가 65 절단 **앞**에서 병합하므로 조회 대상과 절단분은 키가 겹치지 않고, 확인된 책의 키는 조회 대상에서만 나온다. 그래서 `lookup_capped`·`no_match`·`ambiguous`·`search_failed`·`facts_failed`는 이 차감에 **닿을 수 없다.** 확신도 하한에 걸린 바구니만 병합을 거치지 않아 겹칠 수 있고, 그것이 이 런이 닫은 결함 ①의 실제 모양이다 — 같은 책을 한 장에서는 흐릿하게(저확신) 다른 장에서는 또렷하게 읽은 경우.

누적이 수동 집계이므로 **이 변경을 가로지르는 비교에는 안내가 세 줄 필요하다.** ⓐ 판독 품질을 볼 때는 `raw_unidentified_by_measurement`의 `lookup_capped + blank_title` 합을 본다. ⓑ **후보 폭발 추이를 볼 때는 그 합이 아니라 `raw_candidate_count`(접힘 전)를 본다** — 접힘이 지운 몫은 그 값에만 남아 있다. ⓒ **그 합의 절대값은 이 변경을 가로질러 비교하지 않는다** — 접힘 규칙이 함께 바뀌었기 때문이다. 옛 값은 빈 키 판독본을 **하나로 접은 뒤**의 수여서, 책등 20개를 못 읽은 세션이 옛 세계에서는 1이고 새 세계에서는 20이다. ⓐ가 잡아 주는 것은 **바구니 경계가 움직였다는 사실 하나**이지 수의 연속성이 아니다. 이 셋이 없으면 읽는 사람이 `lookup_capped`의 감소를 성능 개선으로, 그 합의 증가를 판독 품질 저하로 읽는다.

**아직 못 고친 한계 다섯.** 재지 않은 것을 잰 것처럼 적지 않는다.

**같은 넷이 아니다.** 접힘 규칙 재설계가 예전 ①②(경계를 안 넘는 접힘 · 빈 키가 한 키로 접힘)를 **닫았고**, 그 과정에서 셋을 **새로 열었다**. 아래 1·2가 예전 ③④ 그대로이고 3·4·5가 새로 열린 것이다. 넷이 다섯이 됐다고 해서 아무것도 안 바뀐 것이 아니다.

1. **`overflow_count`는 확인된 책 절단(50)만 센다.** 미확인 절단(100)의 넘침은 로그에 없다. 응답은 둘을 구분해 내보내는데 로그는 하나만 센다
2. **후보 폭발형 프롬프트 저하는 이 지표로 안 보인다.** 환각으로 후보를 쏟아 내면 초과분이 대부분 `lookup_capped`가 되어 모집단 밖으로 빠진다. "후보 총수 급증"이라는 다른 지표가 필요하고 그 임계는 이 리포가 한 번도 잰 적이 없다. **조회 전 분기가 이 한계의 한 갈래(제목이 빈 노이즈가 쏟아지는 형태)를 덤으로 보이게 만들었지만, 본체 — 그럴듯하게 읽히는 후보를 환각으로 쏟아 내는 형태 — 는 그대로 안 보인다. 닫히지 않았다**
3. **정규화 제목이 서로 다른 판독본은 여전히 안 접힌다.** 흐릿하게 읽은 "해리포터와 마법사돌"과 또렷하게 읽은 "해리 포터와 마법사의 돌"은 정규화해도 다른 문자열이라 키가 다르고, 경계를 넘는 접힘은 **키가 같은 경우**만 접는다. 유사도로 접으려면 `titleSimilarity` 임계가 필요한데 그 임계는 이 리포가 잰 적이 없다
4. **한 ISBN에 성격이 다른 키가 모이면 분자가 실제보다 작아진다.** 확인 키를 `dedupeByIsbn` **전**에서 모으므로, 서로 다른 책이 같은 ISBN으로 오판독되면 그 오판독 키까지 차감에 쓰인다. 줄이는 길은 `MATCH_THRESHOLD`이고 이 런은 상수를 안 건드렸다
5. **`lookup_capped`가 예전과 같은 뜻이 아니다.** 정규화 제목이 빈 후보가 이제 조회 대기열에 들어가지 않아 이 칸이 아니라 `blank_title`로 간다. 위 2를 실제로 닫으러 갈 때 이 칸을 후보 폭발의 신호로 쓸 수 있는지는 **그때 다시 판정해야 한다** — 이 런은 그 판정을 하지 않았다. **여기 적혀 있던 다른 이유 하나는 거짓이었다** (07 내장 리뷰): "확인된 책과 키가 겹치는 절단 판독본이 그 칸에서도 빠진다"고 적었는데, 병합이 절단보다 먼저라 그런 항목은 **존재할 수 없다.** 사람이 에스컬레이션에서 "그 칸이 줄어드는 대가를 알고 고른다"고 판단한 대가는 **실제로는 치러지지 않았다** — 판단의 근거가 틀렸던 것이고, 그 사실을 지우지 않고 여기 남긴다

**누적과 롤백은 여전히 수동이다.** 이벤트 로그가 표준 출력 단일 축이라 "20건 누적 시 프롬프트 롤백"을 자동으로 수행할 주체가 없다. 이 계측은 **수동 집계가 가능해지는 데까지**만 간다 — 외부 관측 시스템과 롤백 트리거는 저장소 도입(ADR-007)과 같은 시점의 일이다.
...
| Rate Limiting | **`/api/analyze` 한 라우트에 IP당 분당 20회, 초과 시 429 + `Retry-After`** (TR-013, ADR-012). 인스턴스별 in-memory 고정 윈도라 **실효 상한이 인스턴스 수에 비례하고 그 수는 재지 않았다.** 키는 `x-vercel-forwarded-for` → `x-forwarded-for` 순으로 읽는데, **후자만 있는 환경에서는 왼쪽 끝을 요청자가 직접 쓸 수 있어 이 통제가 켜진 채로 아무도 막지 못한다.** 고정 윈도라 경계에서 짧은 구간에 최대 40회가 지나간다. **나머지 라우트 넷은 안 덮인다** | [MVP] |
...
- 다만 **멱등한 것과 공짜인 것은 다르다.** 재시도는 데이터를 망가뜨리지 않지만 비용은 그대로 다시 발생한다. 그래서 자동 재시도는 1회로 묶고, 사용자가 누르는 재시도 버튼에도 상한을 둔다 — `/api/analyze` 재시도는 **세션당 3회**, 간격은 0초 → 5초 → 15초로 벌린다(FR-010). 상한을 소진하면 버튼을 비활성화하고 "잠시 후 다시 시도해 주세요"로 안내한다. 레이트 리밋이 없는 MVP에서 이것이 사용자 쪽 비용 증폭을 막는 유일한 장치다.
...
- **추천 호출은 강행 플래그를 받는다** (TR-010, ADR-008). 라우트가 `irrelevantStreak >= 2`로 무관 판정을 무시하기로 했으면 그 사실을 **첫 호출부터** 프롬프트에 실어야 한다 — 그러지 않으면 모델이 관련성 규칙대로 추천을 비워 보내고 강행이 `200` + 추천 0건으로 끝난다. 세 가지가 규약이다: **재호출이 아니라 첫 호출의 옵션**이라 호출 수가 늘지 않는다(6.1 표 유효) · 화이트리스트 위반으로 교정 재요청이 일어나도 **강행 여부는 유지된다** · 강행이어도 응답의 `relevant` 값은 **모델이 준 그대로** 올려 보낸다(서버가 무시하기로 한 것은 값의 효력이지 값이 아니다).
- 프롬프트 문구의 소유자는 `lib/prompts.ts` 하나다. 서비스 계층(`services/anthropic.ts`)은 플래그만 넘기고 문자열을 조립하지 않는다 — 두 곳에서 문장을 만들면 다음에 고칠 때 한 곳만 고친다.
...
- 이것이 없으면 알라딘이 완전히 다운됐을 때 한 요청이 후보 65건 × (호출 1 + 재시도 1) = **최대 130회의 실패 호출**을 던지고, 12s 대조 예산을 타임아웃으로만 소진한 뒤에야 끝난다. 이미 죽은 서비스를 계속 두드리는 것은 우리에게도 상대에게도 손해다.
...
**`/api/analyze` 한 라우트에 도입했다** (TR-013 · ADR-012). IP당 분당 20회, 초과 시 429와 `Retry-After` 헤더, 클라이언트는 지수 백오프. 카운터는 **인스턴스별 in-memory 고정 윈도**이고 상태는 `globalThis`에 건다 — 그 결정과 ADR-003 무상태 전제와의 관계는 ADR-012가 적는다.

**이 리밋이 못 막는 것을 여기 직접 적는다** (ADR을 가리키기만 하지 않는다):

- **실효 상한은 `20 × 인스턴스 수`다.** 인스턴스 수는 Vercel이 정하고 **우리가 재지 않았다** — 그래서 실효 상한의 숫자를 적지 않는다
- **콜드 스타트가 카운터를 0으로 되돌린다.** 배포 직후·저트래픽 구간에서 특히 그렇다
- **분산된 공격자는 막지 못한다** — 이것은 6.6의 방어 3단 중 ③이고 ①②를 대체하지 않는다
- **키가 플랫폼 전제 위에 있다.** `x-vercel-forwarded-for`를 먼저 읽고 없으면 `x-forwarded-for`로 떨어지는데, **후자만 있는 환경에서는 왼쪽 끝을 요청자가 직접 쓸 수 있다.** 그러면 이 통제는 **켜진 채로 아무도 막지 못하고**, 위조 키로 추적 상한을 채워 신규 사용자를 공유 버킷으로 미는 증폭까지 가능하다. 엔터프라이즈 `Trusted Proxy`를 켜는 날에도 이 전제가 무너진다
- **추적 상한(50,000)이 포화되면 새 키가 공유 버킷을 함께 쓴다.** 지속적 대규모 유입 중에는 신규 사용자가 실제로 막히고, **세대가 바뀌어도 회복이 보장되지 않는다**
- **고정 윈도라 경계 버스트가 있다** — 경계를 걸치면 짧은 구간에 최대 40회. 장기 평균은 분당 20회다
- **`RATE_LIMIT_MAX_TRACKED_KEYS`(50,000)와 `RATE_LIMIT_SHARED_MAX_REQUESTS`(200)는 재지 않은 값이다.** 인스턴스당 실제 메모리를 관측한 적이 없다

인스턴스 간 카운터 공유가 필요해지면 Supabase로 채운다 (ADR-007). 그때 위 첫 셋과 여섯째가 함께 풀린다 — 넷째는 플랫폼 층의 문제라 그때도 남는다.
...
- **다섯 줄이 한 세트다.** `typecheck`를 빼면 `tsc --noEmit`이 도는 곳이 아무 데도 없다 — `next build`도 타입을 보지만 그때는 이미 빌드까지 간 뒤다. 하네스 어댑터의 `compile` 스테이지가 소비하는 것도 이 명령이다.
- `audit`은 6.5·9번의 CI 게이트와 같은 것이다. 빼고 돌리면 audit 게이트가 문서에만 있고 실제로는 검증되지 않는다. `npm audit --audit-level=high`로도 같다.
...
- **Vercel 서버리스** — 요청당 실행 시간 상한(`/api/analyze`는 60s로 설정). 인스턴스 간 메모리 공유 불가라 레이트 리밋·서킷 브레이커·캐시를 **인스턴스를 가로질러서는** 인메모리로 구현할 수 없다. **이 문장은 지우지 않는다** — 왜 있었는지가 ADR-012의 전제이기 때문이다. 다만 세 항목이 같은 운명은 아니다: 레이트 리밋은 **인스턴스별로 세는 것을 받아들이고** MVP에 들어갔고(TR-013 · ADR-012), 그 대가로 실효 상한이 인스턴스 수에 비례한다. 캐시와 인스턴스 간 브레이커는 여전히 공유 스토어(Supabase, ADR-007)를 기다린다.
- **알라딘 OpenAPI** — 일 5,000회 한도. 후보 상한(65건)이 ItemSearch·ItemLookUp 두 단계를 각각 태우면 하루 약 19세션이 실질 상한이며, 이는 일 요청 100건 가정보다 **먼저 닿는다.** **이 19는 분석 요청만 센다** — `/api/books/resolve`는 별개 요청이고 자기 예산·자기 브레이커를 가지므로 이 유도에 들어가지 않는다. resolve가 붙으면 실제 하루 처리량은 19보다 적고, 그 수는 아직 재지 않았다. 두 종류를 합산하는 일일 누적 카운터가 없는 것은 아래 수용 부채(레이트 리밋 없음 · ADR-007)다. 캐시 도입(Scale)의 실제 트리거는 트래픽이 아니라 이 한도다. 응답이 XML/JSON 혼재이고 쪽수·독자평점이 누락된 레코드가 존재한다.
...
| 알라딘 일일 호출 | 5,000회/일. ItemSearch 65건 × 1~2회 + ItemLookUp 최대 65건 × 1~2회 = **세션당 최대 260회 → 하루 약 19세션**에서 한도 도달 (상한을 두기 전에는 무제한이었다). **이 값은 분석 요청만 센다** — `/api/books/resolve`는 별개 요청이라 이 유도 밖이고, 합산 카운터는 수용 부채다 (ADR-007) | 일일 호출 수 카운트 | ISBN 키-값 캐시 도입 | [Scale] |
| 요청 본문 크기 | 리사이즈 후 5장 약 3MB | 413 응답 | JPEG 품질 0.85 → 0.7 하향 | [MVP] |

**세션당 260회는 산문의 계산식이 아니라 코드가 잠그는 계약이다.** 그 값은 `lib/env.ts`의 상수에서 **유도된다** — 조회 전 후보 상한 `MAX_CANDIDATES_FOR_LOOKUP`(65)이 **ItemSearch와 ItemLookUp 두 단계를 각각** 최대 2회씩 태우므로 `65 × 2단계 × 2회 = 260`이다. 상한 자체를 `lib/env.ts`에 이름 있는 상수로 두고, **유도한 값과 그 상한이 같은지 회귀 테스트로 고정한다.** 넘지 않는지가 아니라 같은지다 — `≤`로 두면 선언만 올려서 검사를 통과시킬 수 있고, 그러면 그 선언은 검증되지 않은 상한으로 돌아간다. 누군가 65를 200으로 올리는 순간 알라딘 일일 한도(5,000회)가 세션 6회에 소진되는데, 지금은 그것을 막는 것이 이 표의 산문뿐이다 — **문서에만 있는 상한은 검증되지 않은 상한이다.** 이 게이트는 인스턴스 간 카운터가 아니라 **한 요청 안의 구조적 상한**이므로 무상태 전제(ADR-003)와 충돌하지 않는다. 일일 누적 카운트와 ISBN 캐시는 여전히 `[Scale]`이고 Supabase 도입 시점의 몫이다 (ADR-007).

> **유도가 한 번 바뀌었다 (2026-09-05).** 예전에는 `(80 + 50) × 2` 로 적었고 그 식의 둘째 항이 확인 상한 `MAX_IDENTIFIED_BOOKS`(50)였다. **실행 경로가 그렇지 않았다** — `app/api/analyze/route.ts` 가 `lookupFactsMany` 를 `capIdentified` 보다 먼저 부르므로 ItemLookUp 은 50건이 아니라 조회 전 후보 상한만큼 나갔고, 최악값이 260이 아니라 320이었다. 회귀 테스트가 잠그고 있던 식도 실행 경로가 아니라 이 산문이었다 — **문서와 테스트가 같이 틀려 서로를 지지하고 있었다.** 답은 두 단계에 같은 수를 태우는 것이고, 그래서 후보 상한이 80에서 65로 내려갔다. `MAX_IDENTIFIED_BOOKS` 는 조회 **후**에 도는 표시 상한이므로 이 유도의 항이 아니다.
...
| **공개 URL 남용으로 인한 비용 폭주** | 운영/보안 | 중 | 높음 | 인증도 레이트 리밋도 없는 엔드포인트가 장당 45원짜리 모델을 부른다. 세션당 가드레일로는 **총량** 폭주를 잡지 못한다. 방어 3단: ① Anthropic 콘솔 spend limit(배포 전 체크리스트) ② `SERVICE_ENABLED=false` 긴급 차단 ③ **TR-013 레이트 리밋 — `/api/analyze`에 도입 완료** (ADR-012). 인스턴스별이라 실효 상한이 인스턴스 수에 비례하고 그 수는 재지 않았다. 나머지 라우트 넷은 안 덮인다 | **일일 총 호출 수**가 평소의 3배를 넘거나, 한 `session_id`에서 분당 10회 이상 요청 |
...
| 모델이 후보를 대량으로 쏟아내 외부 호출이 증폭 | 기술/운영 | 하 | 중간 | 추출 스키마에 사진당 60건 상한, 조회 전 총 65건 상한(`confidence` 내림차순), 미확인 100건 상한. 상한이 없으면 판독 한 번의 이상 동작이 알라딘 일일 한도를 한 요청에 소진시킬 수 있다 | 세션당 알라딘 호출 수가 260회에 근접 |
...
| ~~레이트 리밋 없음~~ → **`/api/analyze` 하나만 덮인다** | 가장 비싼 경로(Claude vision)가 걸린 라우트를 먼저 덮었다. `recommend`·`books/resolve`·`mood/questions`·`events` **넷은 안 덮인다** — 넷이 안전해졌다고 읽으면 안 된다 (TR-013 · ADR-012) | 네 라우트 중 어느 하나에서 남용이 관측될 때, 또는 링크를 공개 배포할 때 |
| 레이트 리밋 카운터가 인스턴스별 | 인스턴스 간 공유에 외부 스토어가 필요해 무상태 전제와 충돌. 인스턴스별로 세는 것을 받아들이고 **그 한계를 문서로 적는 쪽**을 골랐다 (ADR-012) | 인스턴스 수가 늘어 실효 상한이 의미를 잃을 때. 카운터 스토어는 Supabase (ADR-007) |
