...
- **알라딘 OpenAPI** — 일 5,000회 한도. 후보 상한(65건)이 ItemSearch·ItemLookUp 두 단계를 각각 태우면 하루 약 19세션이 실질 상한이며, 이는 일 요청 100건 가정보다 **먼저 닿는다.** 캐시 도입(Scale)의 실제 트리거는 트래픽이 아니라 이 한도다. 응답이 XML/JSON 혼재이고 쪽수·독자평점이 누락된 레코드가 존재한다.
...
| 알라딘 일일 호출 | 5,000회/일. ItemSearch 65건 × 1~2회 + ItemLookUp 최대 65건 × 1~2회 = **세션당 최대 260회 → 하루 약 19세션**에서 한도 도달 (상한을 두기 전에는 무제한이었다) | 일일 호출 수 카운트 | ISBN 키-값 캐시 도입 | [Scale] |
| 요청 본문 크기 | 리사이즈 후 5장 약 3MB | 413 응답 | JPEG 품질 0.85 → 0.7 하향 | [MVP] |

**세션당 260회는 산문의 계산식이 아니라 코드가 잠그는 계약이다.** 그 값은 `lib/env.ts`의 상수에서 **유도된다** — 조회 전 후보 상한 `MAX_CANDIDATES_FOR_LOOKUP`(65)이 **ItemSearch와 ItemLookUp 두 단계를 각각** 최대 2회씩 태우므로 `65 × 2단계 × 2회 = 260`이다. 상한 자체를 `lib/env.ts`에 이름 있는 상수로 두고, **유도한 값이 그 상한을 넘지 않는지 회귀 테스트로 고정한다.** 누군가 65를 200으로 올리는 순간 알라딘 일일 한도(5,000회)가 세션 8회에 소진되는데, 지금은 그것을 막는 것이 이 표의 산문뿐이다 — **문서에만 있는 상한은 검증되지 않은 상한이다.** 이 게이트는 인스턴스 간 카운터가 아니라 **한 요청 안의 구조적 상한**이므로 무상태 전제(ADR-003)와 충돌하지 않는다. 일일 누적 카운트와 ISBN 캐시는 여전히 `[Scale]`이고 Supabase 도입 시점의 몫이다 (ADR-007).
