...
| 확인 경계를 넘는 접힘 | 확인된 책과 키가 겹치는 절단 판독본이 **어느 칸에도 안 남는다** | **안 된다** — 분해표에서 완전히 사라지므로 어떤 합으로도 복원되지 않는다 |
...
5. **`lookup_capped`가 더는 "상한에 밀린 판독본 전부"가 아니다.** 확인된 책과 키가 겹치는 판독본이 그 칸에서도 빠진다. 위 2를 실제로 닫으러 갈 때 이 칸을 후보 폭발의 신호로 쓸 수 있는지는 **그때 다시 판정해야 한다** — 이 런은 그 판정을 하지 않았고, 사람이 범위를 줄이며 그 대가를 알고 골랐다
