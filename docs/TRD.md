...
| 필드 | 무엇 | 기준 |
|------|------|------|
| `raw_guardrail_denominator` | 확인된 책 + 분자 대상 미확인 = **가드레일 분모** | 절단 전 · 접힘 후 |
| `raw_unidentified_guardrail_count` | `low_confidence` + `blank_title` + `no_match` + `ambiguous` = **분자** | 절단 전 · 접힘 후 |
| `raw_unidentified_by_measurement` | 계측 사유 일곱의 분해 | 절단 전 · 접힘 후 |
...
**아직 못 고친 한계 넷.** 재지 않은 것을 잰 것처럼 적지 않는다.

1. **접힘이 확인/미확인 경계를 가로지르지 않는다.** 같은 책의 흐릿한 판독본은 분자로, 또렷한 판독본은 확인으로 가 **한 책이 분자와 분모에 각각 1씩** 든다. 열 권을 두 장에 담아 각각 흐릿+또렷을 내면 전부 확인된 세션인데도 50%로 읽힌다
2. **정규화 제목이 빈 항목이 한 키로 접힌다.** 판독이 무너진 스파인 20개가 응답에는 카드 20장, 계측에는 1건이다 — **판독 품질이 가장 나쁜 세션에서 분자가 가장 작게 나온다.** 빈 키는 "같은 책"의 증거가 아니라 증거의 부재다
3. **`overflow_count`는 확인된 책 절단(50)만 센다.** 미확인 절단(100)의 넘침은 로그에 없다. 응답은 둘을 구분해 내보내는데 로그는 하나만 센다
4. **후보 폭발형 프롬프트 저하는 이 지표로 안 보인다.** 환각으로 후보를 쏟아 내면 초과분이 전부 `lookup_capped`가 되어 모집단 밖으로 빠진다. "후보 총수 급증"이라는 다른 지표가 필요하고 그 임계는 이 리포가 한 번도 잰 적이 없다
