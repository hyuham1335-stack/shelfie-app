...
**한계 — 셋째 칸이 이미 유력하다는 것을 숨기지 않는다.** `distinct_keys ≈ count`
(NAMING 86/84 · DOC_CODE_DRIFT 18/18)라 **임계를 낮춰도 승격은 안 열린다.** 그런데도
지금 축을 바꾸지 않는 이유는 [[ADR-H026]] 의 이유 ①이 아직 안 풀렸기 때문이다 —
관측의 다수가 `target_role: main` 이라 **승격의 산물이 갈 곳조차 없다.** 결론을 미리
적는 것과 결론대로 지금 고치는 것은 다르고, 이 리포는 여는 것과 닫는 것을 한 증분에
섞지 않는다.
...
관련: [[ADR-H026]](축을 안 바꾼 이유 셋 · `in_baseline` 이 같은 모양으로 지났다) ·
[[ADR-H007]](못 잰 것을 채우지 않는다 — 단위의 한계를 칸 이름이 말한다) ·
[[ADR-H032]](재는 장치는 이미 있고 읽는 쪽만 없었다 — 같은 논법)
