...
**트레이드오프**: 설계자가 `sources`를 채워야 하고, 필요 없는 파일을 넣으면 접두부만 커진다. 채우지 않으면 기존 동작이므로 롤백 비용은 0이다. **런 #6에서는 켜지 않는다** — 런 #6은 [[ADR-H008]]의 A/B이고 두 변경을 겹치면 귀속이 불가능해진다. 런 #7이 이 결정의 실측을 준다.
