...
**설계 시 실제로 만나는 제약**: 상한 60,000자를 한 파일이 혼자 채울 수 있다. 이 리포에서 `src/services/anthropic.ts`가 **53,091자로 상한의 88%**이고, `src/lib/prompts.ts`(18,405자)와 같은 step에 넣으면 71,496자로 `exit 2`다. **상한이 레이어 분할을 강제한다** — SKILL.md 원칙 1(scope 최소화)과 같은 답을 내지만 그것이 우연인지 구조인지는 아직 모른다. 상한이 설계를 밀어내는 사례가 쌓이면 60,000이라는 값 자체를 다시 봐야 한다.
