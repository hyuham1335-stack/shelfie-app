...
**후기 (2026-09-10, F).** `keep-paths.txt` 는 **팁에서 지웠다** — 일회성 매니페스트라 클론에게는 값이 없고, 팁에 두면 *"내 리포는 이 파일들로 이뤄져야 한다"* 로 오독된다. **이력에는 그대로 있고** 이 문단이 가리키는 것도 그 커밋이다 ([[ADR-H039]] 결정 2).
...
### ADR-H038: 템플릿은 자기 스택을 선언하고, 픽스처는 자기 스택을 선언한다

**날짜**: 2026-09-10 · **상태**: 수용됨 · **세션**: E (코어 스크럽)

**맥락**: [[ADR-H037]] 이 만든 추출본은 `harness/config.json` 이 `"shelfie"` 와
`adapter: nextjs-ts` 를 담은 채였고, 그래서 `doctor` 가 빨간불이었다 —
`nextjs-ts.json` 의 `requires` 가 `package.json` 을 요구하는데 템플릿에는
애플리케이션 스택이 없다. D 는 이 실패를 미리 적고 **E 로 넘겼다.**

---

**결정 1 — 템플릿 자신은 스택 중립 어댑터를 쓰고, `nextjs-ts` 는 예시로 남는다.**

`harness/adapters/self-python.json` 을 신설해 `config.adapter` 가 그것을 가리키게
했다. 러너는 `python`(화이트리스트 안), `requires` 는 `scripts/harness.py` 와
`harness/phases/`, `full` 스테이지는 `-m pytest scripts/` 다. **`requires` 를 빈
배열로 두지 않았다** — 아무것도 재지 않는 전제조건은 통과가 아니라 검사 부재이고,
그 구분이 이 리포의 규율이다 ([[ADR-H007]]).

**`nextjs-ts.json` 과 `profiles/nextjs-ts/` 는 지우지 않았다.** 클론하는 사람이 볼
「채워진 예시」가 하나도 없으면 `_template.json` 의 빈칸만 남고, 빈칸은 무엇을
채워야 하는지 안 가르쳐 준다. 대신 파일럿 고유값(`src/services/anthropic.ts`
프로브 · 알라딘 서비스 종료 서술 · `shelf.golden.test.ts` · `ADR-003`·`ADR-010`
참조)을 일반 예시로 바꿨다. **규율은 남기고 사례만 갈았다** — `on_missing` 을
warn 으로 둘지 fail 로 둘지는 실측이 정한다는 문장([[ADR-H027]])은 그대로 있다.

**넷째 원인이 있었고 D 는 그것을 예측하지 못했다.** 어댑터를 갈아도 `doctor` 는
빨갰다 — `filter-repo` 가 **브랜치 이름을 그대로 물고 왔다**
(`refactor-core-drops-the-executor`). `config.vcs.base_branch` 는 `main` 이므로
`_check_vcs` 가 FAIL 이다. `git branch -m main` 으로 닫았다. D 의 예측 표는
*"어댑터 전제조건"* 하나만 원인으로 적었고, **한 증상에 원인이 둘이었다.**
예측이 틀린 것이 아니라 **덜 적힌 것**이고, 그 차이를 여기 적어 둔다.

---

**결정 2 — 테스트 픽스처의 스택은 리포 자신의 스택과 별개 사실이다.**

`test_harness.py`·`test_pipeline.py` 의 픽스처는 실물 `harness/config.json` 을
복사한다. *"실물이 바뀌면 이 테스트가 먼저 깨진다"* 는 규율이고 값이 있다. 그런데
그 픽스처가 세우는 리포는 **Next.js 모양**이다 — `package.json`·`src/app/**`·
`vitest`·`src/lib/match.ts`. 파일럿이 Next.js 리포였기 때문에 여태 두 사실이
우연히 일치했을 뿐이다.

어댑터를 갈자 그 우연이 끝났고 `test_pipeline.py` 가 **290건** 빨개졌다. 뿌리는
하나다 — 픽스처가 복사한 config 는 `self-python` 을 가리키는데 픽스처는 그 어댑터
파일을 안 복사한다.

**픽스처가 자기 어댑터를 선언하게 고쳤다.** 복사는 그대로 두고 복사 직후
`cfg["adapter"] = "nextjs-ts"` 를 박는다. 스키마·역할·계약 절·리뷰어 라우팅은
여전히 실물과 같이 움직이고, **스택만 픽스처가 자기 것을 말한다.**

대안이던 *"픽스처를 파이썬 리포로 바꾼다"* 는 버렸다. `src/` 를 참조하는 줄이 두
테스트 파일에 **81줄**이고, 그것까지 갈면 이 증분이 스크럽이 아니라 재설계가 된다.
같은 이유로 **`config.roles[].owns` 의 `src/**` 모양도 그대로 뒀다** — `src/` 는
파일럿의 고유명사가 아니라 관례다.

---

**결정 3 — 자물쇠는 「파일럿의 고유명사」를 잡고 「스택 이름」은 안 잡는다.**

`TemplateHasNoPilotNamesTest` 의 단어 목록은 `shelfie`·`aladin`·`anthropic`·
`supabase`·`vercel` 다섯이다. **`npm`·`next` 를 넣지 않았다.**

그 둘은 `adapters/adapter.schema.json` 의 러너 화이트리스트 enum 과
`_template.json` 의 `script_manifest` 설명에 **정당하게** 있다. 넣으면 자물쇠가
매번 거짓 실패하고, 거짓 실패하는 자물쇠는 다음 사람이 지운다. [[ADR-H037]] 이
`dangerously-skip-permissions` 검사를 *"코드와 그 코드를 설명하는 산문을 같은
것으로 셌다"* 며 실행 가능한 파일로 좁힌 것과 **같은 자리**다 — 그때는 대상을
좁혔고 이번에는 어휘를 좁혔다.

그래서 `harness.py`·`pr.py` 산문의 `npm` 언급은 **손으로** 러너 중립 표현으로
고쳤고 기계 검사는 안 걸었다. 기계가 못 잡는 것을 잡는 척하지 않는다.

`docs/harness/**`·`CLAUDE.md`·`calibration.json`·`keep-paths.txt` 는 자물쇠에서
뺐다 — **세션 F 가 다시 쓰는 자리**이고, E 가 지금 통과시키면 F 의 범위를
선점한다.
...
[[ADR-H037]] 이 *"옮기면서 동시에 고치면 회귀가 났을 때 어느 쪽이 원인인지 못
가른다"* 며 E 로 넘긴 자리다. `runtime.resolve_tz()` 가 `HARNESS_TZ`(형식
`+09:00`)를 읽고, 없으면 **시스템 로컬**로 간다. 읽을 수 없는 값은 로컬로 폴백하되
`RuntimeWarning` 을 남긴다 — 시각은 게이트가 아니라 죽이지 않고, 조용히 넘기지
않는 것은 [[ADR-H007]] 과 같다.

**하드코딩된 +09:00 은 이 리포를 만든 사람의 시간대이지 클론하는 사람의 것이
아니다.** 시각이 원장·영수증·보고서의 축이라, 조용히 틀리면 그 사람의 기록이 전부
남의 시간으로 남는다.

---

**실측 (2026-09-10)**: 추출본 `pytest` **821 통과 · 1 실패 · 2 skip** — D 가
넘긴 셋 중 둘이 닫혔고 남은 하나는 `TestDoctorRemote` 다(리모트를 붙이는 G 몫).
`doctor` **exit 0**(경고 5건 — `cmd:null` 스테이지 여섯 · 스테이지 명령 검사 안 함 ·
어댑터 `verified:false` · 테스트 수를 셀 수 없음 · 캘리브레이션). **경고는 남아야
맞다** — 스킵됨은 통과가 아니다. 고유명사 게이트 **0건**.

**트레이드오프**:

- **어댑터가 하나 는다.** `self-python` 은 이 템플릿 자신만 쓰는 어댑터이고,
  프로젝트를 시작하는 사람은 `init` 으로 갈아치운다. 그래도 **`doctor` 가 실물
  위에서 무언가를 실제로 재게** 하는 값이 그보다 컸다 — 빈 리포에서 통과하는
  `doctor` 는 [[ADR-H003]] 이 추출을 미룬 바로 그 이유다
- **픽스처가 실물에서 한 칸 멀어졌다.** `adapter` 만큼은 실물이 바뀌어도 픽스처가
  안 깨진다. 그 칸을 실물과 묶어 두는 값보다, 두 리포의 스택이 다르다는 사실을
  픽스처가 **선언하게** 하는 값이 컸다
- **`nextjs-ts` 예시가 검증되지 않은 채 남는다.** `verified: false` 이고 템플릿에는
  그것을 완주시킬 앱이 없다. 이유를 문서에 적는 것이 [[ADR-H037]] 이 어댑터
  `verified` 에 대해 정한 방침 그대로다

**재검토 시점**: 세션 G 가 push 하기 직전. 그때 `TemplateHasNoPilotNamesTest` 의
제외 목록 넷 중 몇이 F 를 거쳐 지워졌는지가 이 결정의 채점표다.

관련: [[ADR-H037]](추출과 실행기 절단 — 이 세션이 받은 빨간불의 출처) ·
[[ADR-H031]](같은 수법을 코어의 스택 이름에 먼저 썼다) · [[ADR-H007]](모르는 것을
모른다고 적는다) · [[ADR-H027]](면제는 통과가 아니다)
...
### ADR-H040: 마지막 빨간불은 리모트가 아니라 push 가 푼다 — 그리고 안 돌린 게이트는 안 돌렸다고 적는다

**날짜**: 2026-09-10 · **상태**: 수용됨 · **세션**: G (게이트 · push)

**맥락**: D·E·F 가 로컬에서 추출본을 만들고 씻고 문서를 다시 썼고, 셋 다
*"리모트를 붙이면 `TestDoctorRemote` 가 풀린다"* 를 G 에 넘겼다
([[ADR-H037]] 의 빨간불 표 · `ROADMAP` §6 항목 36 의 G 행). 이 세션이 받은 것은
빨간불 **1건**과 push **1회**이고, 되돌릴 수 없는 동작이 그 안에 있다.

---

**결정 1 — 순서가 뒤집힌다. `remote add` 는 빨간불을 안 푼다.**

`cli._check_remote` 는 리모트 **이름**만 보지 않는다. `refs/remotes/{remote}/{base}`
를 `rev-parse --verify` 로 확인하고, 없으면 `WARN` 이다:
...
대상 리포는 **실 ref 가 없는 빈 리포**였으므로 `origin/main` 이라는 원격 추적 ref
자체가 존재할 수 없었다. 그래서 초록불로 가는 시점은 `remote add` 가 아니라
**push + fetch 직후**다.

```mermaid
flowchart LR
    A["게이트 1~4<br/>빨간불 1건을 안은 채"] --> B["remote add<br/>여전히 WARN"]
    B --> C["push -u origin main"]
    C --> D["fetch<br/>refs/remotes/origin/main 생성"]
    D --> E["TestDoctorRemote 초록<br/>doctor 경고 5 → 4"]
```

**인계 메모가 틀린 것이 아니라 한 칸 덜 적혔다.** [[ADR-H038]] 이 D 의 예측 표에
대해 *"예측이 틀린 것이 아니라 덜 적힌 것"* 이라고 가른 것과 같은 자리이고, 같은
구분을 여기 한 번 더 쓴다. 실무 결과도 같다 — **게이트를 빨간불 1건을 안은 채
통과시키고 push 한 뒤에 초록을 확인**하는 순서가 강제된다.

**결정 2 — 게이트 ⑤(01~04 실물 완주)는 안 돌렸고, 안 돌렸다고 적는다.**

사람이 이 세션의 「클론 실물 완주」 범위를 **기계 검사만**으로 정했다. 게이트 ⑤는
**미실행**이고 **통과가 아니다.** [[ADR-H013]] 이 *"1~4번은 전부 정적 검사이고,
정적 검사만으로 통과시키면 「검증한 것만 승격한다」가 무너진다"* 고 적은 바로 그
자리이므로, 이 결정은 **게이트를 낮추는 것이 아니라 채점표에 빈칸을 남기는 것**이다.

빈칸으로 두는 근거 셋을 적는다. ① 파일럿에서 2026-09-03 에 다섯 줄 전부 통과했고
(P1 · 모델 호출 10회 · `PASS_WITH_GAPS`), ② 클론의 코어는 그 커밋의 후손이며
`scripts/**` 는 D~F 를 통틀어 스택 지식이 빠지는 방향으로만 바뀌었고, ③ 템플릿에는
완주시킬 애플리케이션이 없어 `self-python` 어댑터가 자기 테스트 슈트를 겨눈다.
**셋 다 "안 해도 된다"의 근거가 아니라 "지금 안 해도 위험이 작다"의 근거다.**
ROADMAP 에 후속 항목으로 등록한다.

**결정 3 — 게이트 ②·③은 추출본이 아니라 사본에서 돌린다.**

둘 다 `harness/config.json` 을 임시로 고쳐야 하는데, push 직전의 추출본에 그
변경이 한 번이라도 앉으면 **되돌렸다는 것을 사람이 믿어야 한다.** 스크래치패드
사본에서 돌리고 추출본은 워킹트리를 한 번도 안 건드렸다 — push 전 `git status`
가 비어 있는 것이 그 증거다.
...
| 게이트 | 방법 | 결과 |
|---|---|---|
| ① 고유명사 0건 | `CoreHasNoStackNamesTest`·`TemplateHasNoPilotNamesTest`·`CoreDoesNotImportTheExecutorTest`·`TemplateDocsAreNotDanglingTest` | **5 통과** |
| ② 스택 교체 무변경 | `_template.json` 에서 `gate2-npm` 신설 → `config.adapter` 만 교체 | `doctor` exit 0 · `lint-phases` exit 0 · `gate --replay` **exit 0 · `PASS_WITH_GAPS`** · `scripts/**` **0줄** |
| ③ 언어 교체 무변경 | `project.language`·`contract.sections`·`templates/contract.md` 만 영어로 | `doctor` exit 0(「계약 절 ↔ 템플릿」 PASS) · `scripts/**` **0줄** |
| ④ 그린필드 | 픽스처 `greenfield-zero-tests` | `PASS_WITH_GAPS` · gap 에 `tests_ran_zero` |
| ⑤ 실물 완주 | — | **미실행** (결정 2) |

`pytest` 는 push 전 **820 통과 · 1 실패 · 2 skip**(F 의 기록과 정확히 일치)에서
push 후 **821 통과 · 0 실패 · 2 skip** 으로 갔다. `doctor` 경고는 **5건 → 4건**
(VCS 하나가 PASS 로 올라갔다). **클론 실측**: 커밋 **190** · 파일 **70** ·
`pytest` **821 통과 · 2 skip** · `doctor` **exit 0**(경고 4건) ·
`lint-phases` **exit 0**.

**게이트 ②가 부수적으로 둘을 증명했다.** ① 손으로 쓴 어댑터의
`test_report.format: "junit"` 오타를 `doctor` 가 잡았다(허용값은 `junit-xml`) —
어댑터 스키마가 선언일 뿐 아니라 **실제로 문다**. ② `script_manifest` 를 선언하자
「스테이지 명령」이 `WARN`("검사 안 함") → `PASS` 로 올라갔다 — [[ADR-H031]] 이
`NODE_RUNNERS` 를 코어에서 어댑터 선언으로 내린 뒤, **코어가 그 선언을 실제로
읽는다**는 것이 다른 스택 위에서 처음 확인됐다.

**채점표 — push 직전에 읽으라고 지정된 재검토 항목 셋**:

| ADR | 채점 대상 | 결과 |
|---|---|---|
| [[ADR-H037]] | D 가 적은 빨간불 셋 중 몇이 남았나 | **1건**(`TestDoctorRemote`) — 그리고 그 하나는 예측대로 G 가 닫았다 |
| [[ADR-H038]] | 제외 목록 넷 중 몇이 F 를 거쳐 지워졌나 | **셋 지워지고 하나 남음**(`scripts/test_harness.py` 자기 참조 · 영구) |
| [[ADR-H039]] | 위 항목이 이 결정으로 닫히는가 | **닫혔다** |

[[ADR-H037]] 이 push 앞에서 다시 읽으라고 지정한 문단(*"새로 공개되는 정보는 0
이다"*)도 확인했다 — `shelfie-app` 이 이미 public 이므로 이력 보존이 새로 노출하는
것은 없다.

**트레이드오프**:

- **게이트 ⑤가 빈칸이다.** 템플릿을 클론한 사람은 *"이 하네스가 실물 기능을
  완주시킨 적이 있는가"* 를 파일럿의 기록으로만 볼 수 있고, [[ADR-H039]] 가 그
  기록을 팁에서 걷어냈으므로 **템플릿 안에는 증거가 없다.** 대신 이 ADR 이 남는다
- **어댑터는 여전히 `verified: false` 다.** 여덟 런째 자연 실패가 오지 않았고
  일부러 만들지 않기로 한 항목이라 템플릿은 `false` 를 그대로 싣는다
- **push 는 되돌릴 수 없다.** `--force` 를 안 썼고 대상이 빈 리포였으므로
  덮어쓴 것이 없다는 것은 확인되지만, 공개 자체는 되돌아가지 않는다

**재검토 시점**: 템플릿을 실제로 클론해 프로젝트를 시작할 때. 그때 게이트 ⑤의
빈칸이 채워지고, `calibration.json` 의 `state: "unmeasured"` 가 첫 `calibrate` 로
실측이 된다. 둘 다 **클론하는 쪽에서만 채울 수 있는 칸**이다.

관련: [[ADR-H013]](게이트 다섯 줄 — 이 세션이 채점한 대상) · [[ADR-H037]](추출
범위와 빨간불 예측) · [[ADR-H038]](코어 스크럽 — 「덜 적힌 예측」의 원 출처) ·
[[ADR-H039]](문서 재작성) · [[ADR-H031]](스택 이름을 어댑터 선언으로 내린 결정 —
게이트 ②가 그것을 확인했다) · [[ADR-H007]](모르는 것을 모른다고 적는다 — 결정 2)
