...
**그러나 이 증분이 남긴 가장 큰 부채가 드러났다.** 06 이 PR diff 를 위해 커밋을 요구하는데, 지문이 `HEAD + 소유 범위 미커밋 파일 해시`(`state.py:407`)라 **그 커밋이 04 게이트의 영수증을 반드시 낡게 만든다.** `approve` 는 자기 시점 지문을 따로 박아 06 을 통과시키지만 `advance` 는 04 지문과 대조한다 — 그래서 **06 을 지난 런은 `advance --phase 08` 로 닫을 수 없다**(M25). `record --phase 08` 도 미구현이다(M24). **이 런은 08 을 `running` 으로 남긴 채 끝났다.** 순서를 밟지 않고 06~08 을 한 증분으로 지은 대가가 여기서 나왔다 — [[ADR-H015]] 가 감수하기로 한 바로 그 형태다.
