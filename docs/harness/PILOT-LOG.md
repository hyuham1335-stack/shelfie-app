...
### M10. `started_at`은 있고 출력 파일은 없는 상태가 "진행 중"과 "죽었음" 양쪽을 뜻한다 — 비차단, 관측됨
...
1. **`src/app/page.tsx` 상태 머신** — 네 런이 만든 조각(서비스·라우트·컴포넌트)이 처음으로 한 화면에서 만난다. `ambiguous` 후보의 `proof` 공백이 거기서 실제 문제가 된다
2. **이월 보고 4건을 받을 step을 명시적으로 배치한다.** 세 런째 쌓였고 phase 설계가 매번 금지사항으로 막았다
3. **M10** — `RUNNING` 파일. 3단계 병렬 스테이지의 전제 조건이다
4. `MAX_RETRIES = 3`은 **20 step 동안 한 번도 발동하지 않았다.** 실측 없는 상수로 남은 지 네 런째다 — 지우거나, 발동시키는 조건을 실측으로 만들거나 둘 중 하나를 정할 때다
