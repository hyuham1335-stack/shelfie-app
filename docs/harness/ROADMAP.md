...
| M | 무엇으로 확인되나 |
|---|---|
| **M50** | 새 파일을 만드는 런의 `04_gate_report.contract.unmatched` 가 0 이고, `scope.repo_files` 가 `05_trace.repo_files` 와 같다. `scoped` 가 그 런의 새 테스트를 **루프 안에서** 돈다 |
| **M45** | 03·05 봉투에 소유권 표와 제출 규약이 **끝까지** 실린다. 05 제출이 규약 미고지로 반려되는 일이 줄어드는지 — M37·M38 이 답 못 한 자리다 |
| **M46** | 어휘 위반이 나면 **그 리뷰어만** 재제출하고 `attempts` 에 세어진다. 셋 전원 재제출이 사라지는가 |
| **M47** | `events.jsonl` 에 `counter_inc` 가 실재하고, `spent[].reason` 이 형식 반려와 수리를 가른다. **여기서 나온 값이 예산을 가를지의 첫 표본이다** |
| **M48** | `deduped` 가 0 이 아닌 런이 나오는가. 07 이 실제로 선언을 다는지 — **안 달면 이 수정은 배선만 맞은 것이다** |
| **M49** | 07 에서 메인이 고친 지적이 원장에 `repaired` 로 남는가. 거짓 주장이 exit 8 로 막히는가 |

**여전히 자연 실패를 기다리는 셋**(M32·M33·M35)과 어댑터 `verified` 는 이 증분이 건드리지 않았다.
