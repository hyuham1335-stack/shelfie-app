...
## 1. 현재 상태 (3단계 — 01~04 실물 완주 · 05 배선 완료)
...
| **파이프라인 코어** | `scripts/pipeline/{cli,state,adapters,attribution,verdict,contract,gate,trace_contract,review,precheck,ledger}.py` — 8페이즈 실행기. `doctor` · `init --feature` · `next` · `record` · `gate` · `advance` · `retry` · `escalate` · `resume` · `status` · `lint-phases` · **`precheck` · `contract-trace`**. **stdout 은 언제나 단일 JSON 봉투 하나**. 모듈명이 `trace.py` 가 아닌 것은 stdlib `trace` 를 가리기 때문이다 |
| **페이즈 파일** | `harness/phases/{01-plan,02-cross-verify,03-implement,04-gate,05-code-review}.md` — `---` 로 감싼 JSON 프론트매터. **06~08 은 아직 없고** `lint-phases` 가 그 전이를 `FUTURE` 로 드러낸다 — 그 한 줄이 다음 작업의 진입점이고, 페이즈가 늘 때마다 **지우는 것이 아니라 옮긴다**. 01·02·05 의 "제출 형식" 절이 리뷰어 원문 형태와 재제기 어휘를 함께 적는다 ([[ADR-H014]]) |
...
| **진입점** | `.claude/commands/feature.md` (`/feature` — **01~05 까지**. PR 을 만들지 않고 push 하지 않는다) · `.claude/agents/{impl-writer,test-writer,plan-reviewer}.md` (각 3KB 이하 — 소유 경계·제출 형식·금지만 담고 규약은 담지 않는다) |
| 테스트 | `scripts/test_harness.py` (70건) · `scripts/test_execute.py` (176건) · `scripts/test_pipeline.py` (**284건**). 파일럿 앱은 1333건 |
...
32. **05 의 첫 실물 표본이 아직 없다.** 배선은 섰고 단위 검증은 284건이 잠갔지만, **리뷰어를 실제로 띄운 런이 0회다.** 그래서 아직 모르는 것 셋을 적어 둔다 — ① **리뷰어 호출 고정비**(첫 3런의 원장이 만든다) ② `findings_max: 50` 절단이 실제로 걸리는 빈도 ③ `untested_contract_item` 의 **오탐률**. 셋 다 값이 없으므로 상수로 굳히지 않았고 `config.json` 의 `_review_note` 가 그렇게 적고 있다.
