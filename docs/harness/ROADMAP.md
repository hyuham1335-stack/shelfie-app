...
| 테스트 | `scripts/test_harness.py` (70건) · `scripts/test_execute.py` (140건) |
| 파일럿 | `phases/lib-core/` · `phases/services-core/` · `phases/routes-core/` · `phases/app-core/` 각 5 step · `phases/app-shell/` 6 step · `phases/contract-wiring/` · `phases/forced-path/` 각 5 step — **일곱 런 36 step 완주.** 기록은 [PILOT-LOG.md](PILOT-LOG.md) |
...
flowchart LR
    ...
    S2["2단계 · 런 #1~#7 완주<br/>Shelfie lib/ · services/ · app/api/ · components/ · app/ · 계약 배선 · 강행 경로"]
...
| **2. 파일럿** | 첫 실제 프로젝트(**Shelfie**)를 **현재 실행기로** 완주. 부족한 지점을 기록 | 스테이지별 실측 시간 · 테스트 개수 · 린트 위반 기준선 확보 | **런 #1~#7 완주** — `lib/`·`services/`·`app/api/`·`components/`·`app/`·계약 배선·강행 경로 36 step, 36/36 무재시도. 실측은 [PILOT-LOG.md](PILOT-LOG.md) |
...
1~11과 13·15·16·17·18·19·21·22·23·25·27이 완료됐다. `calibrate`가 구현됐고 파일럿 런 #1~#7이 `lib/`·`services/`·`app/api/`·`components/`·`app/`·계약 배선·강행 경로를 완주해 **앱이 업로드에서 추천까지 이어졌고 계약과 코드가 일치한다 — US-003의 마지막 칸이 런 #7에서 닫혔다.** **남은 것은 12·14·28이다** — 셋 다 "대상이 없어서" 또는 "표본이 모자라서" 열려 있고, 실행기 결함(M11·M13·M14)은 2026-09-02에 전부 닫혔다. 20·24·26도 닫혔다. **13은 답이 뒤집혀 다시 열렸다가 닫혔다**(아래). 런 #4에서 UI_GUIDE의 "Claude 생성 텍스트 블록"과 사실·해석 분리(ADR-002)가 처음 화면에서 시험되어 통과했고, ADR-005(`lookup_failed` ≠ `no_match`)도 문구·색·행동 셋 다 갈렸다.
...
28. **접두부에 통합 예산이 필요하다 — 두 결정이 같은 자원을 반대로 당기는데 서로를 모른다.** 런 #7이 드러냈다.
    ...
    **재는 쪽은 2026-09-02에 만들었다 ([[ADR-H010]]).** 실행기가 접두부를 네 조각으로 분해해 `steps[].runs[]`에 남긴다 — `preamble_chars`(접두부 전체) · `guardrail_chars` · `source_chars` · `context_chars`(누적 summary), 그리고 접두부 **밖**인 `step_body_chars`. 나머지(상용구)는 차로 유도된다. step 시작마다 같은 분해를 콘솔에도 찍는다. **상한은 걸지 않았다** — 걸면 그 순간부터 step 설계가 그 값에 맞춰지고, 그러면 그 값이 옳았는지 잴 표본이 더는 생기지 않는다. 막는 대신 **드러낸다.** 값을 정할 표본은 런 #8부터 쌓인다.
    ...
    함께 볼 열린 질문 — **첨부는 읽을 파일에 값을 하고 쓸 파일에는 덜 하는가.** 첨부본은 step 시작 시점이라 세션이 그 파일을 고치면 다시 읽어야 한다. 런 #7에서 step 4(접두부 최소·비용 최대)와 step 2(접두부 최대·비용 최소)가 정반대로 나와 **반례가 이미 있다.** 가설이지 결론이 아니고, 런 #8이 답한다
