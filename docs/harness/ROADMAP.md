...
36. ~~**`harness-template` 추출 — 하네스 자산을 별도 public 리포로 옮긴다.**~~ — **닫힘 (2026-09-10, 세션 A~G · [[ADR-H040]])**. 추출본이 <https://github.com/hyuham1335-stack/harness-template> 에 올라갔다(커밋 190 · 파일 70). **두 리포는 이제 독립이다.** 아래는 원 서술과 세션별 기록을 남긴 것이다. [[ADR-H003]] 이 *"빈 템플릿 리포에서는 `doctor` 가 검사할 실물이 없어 통과가 아무것도 의미하지 않는다"* 를 이유로 추출을 미뤘고 트레이드오프 칸에 **"파일럿 완주 후로 미룬다"** 고 적어 두었다. 그 조건이 채워졌다 — [[ADR-H013]] 이 재정의한 승격 게이트 다섯 줄이 2026-09-03 에 전부 통과했고 파이프라인 런 P1~P7 이 8페이즈를 실물에서 일곱 번 완주시켰다. **일회성 추출이고 이후 두 리포는 독립이다** (동기화 기계를 만들지 않는다). 범위는 8페이즈 + 파이썬 테스트 슈트 + 하네스 문서이고 **순차 실행기(`scripts/execute.py` 계열)는 뺀다** — `claude -p --dangerously-skip-permissions` 를 헤드리스로 띄우는 승인 우회([[ADR-H005]])를 클론하는 사람이 물려받게 하지 않는다. git 이력은 `git filter-repo` 로 보존한다. 대상 리포는 이미 비어 있는 채로 만들어져 있다 — <https://github.com/hyuham1335-stack/harness-template> (2026-09-08 생성 · public · 실 ref 없음).
    ...
    | **C7** | M58·M59 를 닫는다 | C | ~~열림~~ **닫힘 (2026-09-10, [[ADR-H036]] · D 의 첫 결정을 「닫고 간다」로 답했다)** |
    | **D** | 이력 추출 + 리포 골격 | C · C4 | ~~열림~~ **닫힘 (2026-09-10, [[ADR-H037]])** — 커밋 362 → 187 · 파일 69 · 테스트 820 · **수집 에러 0** · 빨간불은 미리 적은 셋 그대로 |
    | **E** | 코어 스크럽 — Shelfie 가 새어 든 15군데 | D | ~~열림~~ **닫힘 (2026-09-10, [[ADR-H038]])** — 추출본 **821 통과 · 1 실패**(G 몫 `TestDoctorRemote` 하나) · `doctor` **exit 0** · 고유명사 게이트 0건 |
    | **F** | 문서 재작성 — ROADMAP·PILOT-LOG·`docs/` 골격·시드 | D | ~~열림~~ **닫힘 (2026-09-10, [[ADR-H039]])** — 제외 목록 **넷 → 하나** · 고유명사 **20건 → 0건** · `pytest` **820 통과 · 1 실패**(G 몫) · `doctor` exit 0 · `lint-phases` exit 0. **G 에 넘기는 것**: `TestDoctorRemote` 1건은 그대로 빨갛다(리모트 몫) · `calibration.json` 은 **미측정 골격**이라 클론이 첫 `calibrate` 를 돌려야 실측이 생긴다 · 빈 골격 일곱은 채워지지 않은 채로 `doctor` 를 통과한다 |
    | **G** | 게이트 5줄 + 클론 실물 완주 + `git push` | E·F | ~~열림~~ **닫힘 (2026-09-10, [[ADR-H040]])** — 게이트 **①~④ 통과 · ⑤는 미실행**(사람이 「기계 검사만」을 골랐다 · 아래 39번) · `push` **1회** (`047fe2d` → <https://github.com/hyuham1335-stack/harness-template>) · 추출본 `pytest` **821 통과 · 0 실패 · 2 skip** · `doctor` 경고 **5 → 4** · **클론 실측** 커밋 190 · 파일 70 · `pytest` 821 통과 · `doctor` exit 0 · `lint-phases` exit 0. **마지막 빨간불은 `remote add` 가 아니라 push 가 풀었다** — `_check_remote` 가 `refs/remotes/origin/main` 을 요구한다 |

    **D 의 첫 결정에 답했다 — 닫고 간다 (2026-09-10, C7).** 세션 C 의 범위는 *"M51~M57"* 이고 그 일곱은 전부 닫혔다. M58·M59 는 그 행이 쓰인 뒤에 열린 것이라 C 가 못 닫았고(여는 것과 닫는 것을 한 증분에 섞지 않는다), 아래 *"열린 결함을 안은 채 템플릿으로 굳히지 않는다"* 는 **결함 목록이 아니라 성질**을 말하므로 **D 에 들어가기 전에 둘을 어떻게 할지 정해야 했다** — 닫고 가거나, 템플릿이 그것을 안고 나간다는 사실을 문서에 적거나. **닫고 가는 쪽을 골랐다.** 추출은 일회성이고 push 는 G 에서 한 번뿐이라 되돌릴 수 없는데, 둘 다 수정 범위가 좁아 안고 가는 값이 없었다. 어댑터 `verified: true`(위 14번)와의 차이가 여기다 — 그것은 **자연 실패를 기다리는 항목**이라 닫을 방법이 없고, 이 둘은 있었다.
    ...
    **D 가 같은 수법을 실행기 의존에 썼다** (2026-09-10, [[ADR-H037]]). A 는 스택 이름 목록을 코어에서 **어댑터 선언**으로 내렸고, D 는 트랜스크립트 읽기를 실행기에서 **코어 쪽 모듈**(`scripts/runtime.py`)로 내렸다. 방향만 반대이고 규율은 같다 — *지식은 그것을 소유해야 할 계층에 둔다.* 자물쇠도 나란히 앉는다(`CoreDoesNotImportTheExecutorTest`). **A 의 게이트가 잡지 못하던 종류였다**: 스택 고유명사가 아니라 **계층 방향**이 틀린 것이라, 이름을 세는 검사로는 영영 안 보였다.

    **E 가 닫힌 자리** (2026-09-10, [[ADR-H038]]). 추출본의 `doctor` 를 exit 0 으로 만든 것은 어댑터 교체다 — `harness/adapters/self-python.json` 을 신설해 리포 자신이 그것을 쓰게 했고, `nextjs-ts` 는 **씻어서 예시로 남겼다**(지우면 클론하는 사람에게 채워진 예시가 하나도 안 남는다). **D 의 예측 표는 한 증상에 원인을 하나만 적었다** — 어댑터를 갈아도 `doctor` 는 빨갰고, `filter-repo` 가 브랜치 이름까지 물고 와 `base_branch: main` 과 안 맞았다. 예측이 틀린 것이 아니라 덜 적힌 것이고, 그 구분을 ADR 에 남겼다.

    **F 가 닫힌 자리** (2026-09-10, [[ADR-H039]]). 문서 셋을 **성질에 따라 다르게**
    처리했다 — `ROADMAP.md` 는 재작성(799 → 198줄), `PILOT-LOG.md` 는 빈 골격
    (2,927 → 81줄), `DECISIONS.md` 는 **씻어서 통째로 남겼다**(ADR 을 한 줄도 안 지웠다 ·
    고유명사 15자리만 역할 이름으로 · **숫자는 한 자도 안 고쳤다**). 셋을 같은
    방식으로 다루면 둘 중 하나를 잃는다 — 전부 지우면 *왜 이렇게 생겼는지*를 잃고,
    전부 남기면 *남의 기록*을 물려준다.

    **G 가 닫힌 자리** (2026-09-10, [[ADR-H040]]). 게이트 ①~④ 를 추출본에서
    돌리고 push 한 뒤 **깨끗한 클론에서 다시 확인했다.** 세 가지가 새로 드러났다.
    ① **마지막 빨간불은 리모트를 붙여서 풀리지 않았다** — `_check_remote` 가
    이름이 아니라 `refs/remotes/origin/main` 이라는 원격 추적 ref 를 요구하는데
    대상 리포가 빈 리포였다. **push 하고 fetch 해야 초록이 된다.** D·E·F 가 셋 다
    *"리모트를 붙이면 풀린다"* 고 적었고, 틀린 것이 아니라 한 칸 덜 적힌 것이다
    ([[ADR-H038]] 이 D 의 예측 표에 대해 쓴 것과 같은 구분).
    ② **게이트 ②가 [[ADR-H031]] 을 다른 스택 위에서 처음 확인했다** — `_template.json`
    에서 새 어댑터를 쓰고 `config.adapter` 만 바꾸자 「스테이지 명령」이
    `WARN`("검사 안 함") → `PASS` 로 올라갔다. **코어가 `script_manifest` 선언을
    실제로 읽는다.** 같은 자리에서 어댑터 스키마가 손으로 쓴 오타
    (`format: "junit"`)를 잡았다 — 선언이 무는지도 함께 확인됐다.
    ③ **게이트 ⑤는 빈칸으로 남았다.** 통과가 아니라 **미실행**이고, 39번으로
    등록했다.

    **「데이터를 비운다」와 「파일을 지운다」가 같은 말이 아니었다.** 실측 셋을 전부
    지우자 `doctor` 가 **exit 2** 로 막았다 — `04-gate.md` 가
    `${calibration.derived.background_full_regression}` 을 치환에 쓰는데 파일이
    없으면 해결되지 않는다. `calibration.json` 은 **실측 기록이자 페이즈 파일이
    참조하는 선언** 두 역할을 겸했고, 그 차이를 기계가 가르쳐 줬다. 삭제 대신
    `state: "unmeasured"` 골격을 남겼다.

    **예측을 안 적은 것이 F 의 결함이다.** 실측을 비우자 `pytest` 가 **예상 밖
    넷**을 냈다 — 픽스처가 실물 `calibration.json` 을 복사하고 있었고(D 가
    어댑터에서 만난 것과 같은 자리), 파일럿 원장 168줄의 집계를 고정 기대값으로
    못박은 검사 셋이 대상을 잃었다. 픽스처가 **자기 캘리브레이션을 선언**하게
    고쳤고 원장 검사 셋은 지웠다 — **잃은 것은 일회성 마이그레이션 잠금이지
    동작 잠금이 아니다.** D 는 추출 전에 빨간불 셋을 미리 적어 채점 기준을
    세웠는데 F 는 그러지 않았고, **틀린 예측은 채점이 되지만 없는 예측은
    아무것도 안 남긴다.**

    **그리고 매달린 참조 일곱을 기계가 잡게 했다.** `TemplateDocsAreNotDanglingTest`
    는 `CoreHasNoStackNamesTest` 의 거울상이다 — 그쪽이 *있으면 안 되는 것*을
    잡는다면 이쪽은 *없으면 안 되는 것*을 잡는다. **반대로 자물쇠를 안 건 자리도
    정했다**: 실측 데이터가 비었는지는 잠그지 않는다. 클론하는 사람이 자기 런을
    채우는 순간 거짓 실패하고, 거짓 실패하는 자물쇠는 다음 사람이 지운다
    ([[ADR-H038]] 이 `npm`·`next` 를 어휘에서 뺀 것과 같은 판단이다).

    **그리고 어댑터 교체가 `test_pipeline.py` 를 290건 빨갛게 만들었다.** 뿌리는 하나다 — 픽스처가 세우는 리포는 **Next.js 모양**인데(`package.json`·`src/app/**`·`vitest`) 실물 config 를 복사하니 이제 파이썬 어댑터를 가리킨다. **파일럿이 Next.js 리포였기 때문에 두 사실이 여태 우연히 일치했을 뿐이다.** 픽스처가 자기 어댑터를 선언하게 고쳐 닫았고, `src/**` 를 참조하는 81줄은 건드리지 않았다 — `src/` 는 파일럿의 고유명사가 아니라 관례다.

    **자물쇠의 어휘를 좁힌 것이 이 세션의 셋째 결정이다.** `TemplateHasNoPilotNamesTest` 는 `shelfie`·`aladin`·`anthropic`·`supabase`·`vercel` 다섯만 잡고 **`npm`·`next` 는 안 잡는다** — 그 둘은 러너 화이트리스트와 `script_manifest` 설명에 정당하게 있어서, 넣으면 자물쇠가 매번 거짓 실패하고 다음 사람이 그것을 지운다. [[ADR-H037]] 이 `dangerously-skip-permissions` 검사를 실행 가능한 파일로 좁힌 것과 같은 자리이고, 그때는 **대상**을 좁혔고 이번에는 **어휘**를 좁혔다. 기계가 못 잡는 것(코어 산문의 `npm` 언급)은 손으로 고치고 검사를 안 걸었다.
...
**M58 — 등록만 했고 2026-09-10 에 닫혔다 (아래 C7).** `counter_grant`(`state.py:455`)의 `setdefault(name, {"used": 0, "max": extra})` 는 노드가 없을 때 초기값 `extra` 에 다시 `extra` 를 더해 **상한이 2배**가 된다(직접 호출로 재현: 5 지급 → `max` 10). **오늘 실물 경로로는 도달하지 않는다** — 지급은 `_grant_rounds` 한 곳에서 `round` 에만 일어나고 01 이 그 전에 반드시 라운드를 소모한다(`dca1` 은 지급 시점에 이미 6건을 썼다). 재현이 라이브러리 직접 호출로만 가능하므로 **여는 것과 닫는 것을 섞지 않는 규율을 지켜 등록만 했다.** 초기값을 `0` 으로 내려 닫았다 — `max` 는 `grants` 의 파생값이므로 소모가 없는 상태의 상한은 지급 합계와 같아야 한다.
...
38. ~~**이 리포의 `PILOT-LOG.md` 가 P7 에서 멈춰 있다.**~~ — **닫힘 (2026-09-10)**. `PILOT-LOG.md` 에 「파이프라인 런 P8 — `rate-limit-analyze`」 절을 썼다. **출처와 시점을 절 머리에 명시했다** — 런 보고서와 `_workspace/runs/20260908-1720-dca1/` 의 영수증(`state.json`·`events.jsonl`·`04_gate_report.json`·`RUN-OBSERVATIONS.md`)이 근거이고, 그때 적는 것과 같지 않다는 것을 함께 적었다. 소요 칸만 [[ADR-H032]] 가 사후에 잰 값이다. **늦게 쓴 대가가 실물로 나왔다** — 보고서가 「다음 런에서 바꿀 것」으로 적은 셋 중 ①M56·②M57 은 `RUN-OBSERVATIONS.md` 가 후보로 올려 세션 C 가 집어 갔는데, **③(줄 예산 추정)은 보고서 본문에만 있어 M 번호도 이 목록의 자리도 못 받고 지나쳤다.** 아래 **40번으로 등록한다.** 아래는 닫히기 전의 서술이다.  마지막 절이 *"파이프라인 런 P7 — fold-rule-redesign (2026-09-08)"* 이고 `grep -c "P8"` 이 **0** 이다. P8 은 런 보고서(`pipeline/runs/20260908-1720-dca1.md`)와 이 문서의 §6 「P8 이 확인할 것」·「P8 이 연 하네스 결함 둘」에는 있는데 **파일럿 로그에는 없다.** 잃은 것은 *"그 런이 승격 판단에 준 답"* 이 한 자리에 모이는 성질이고, 다음 런이 P7 뒤를 이으면 그 사이가 영영 빈다. **세션 F 가 이 항목을 등록만 하고 넘긴다** — F 의 범위는 템플릿의 문서이고, 파일럿 로그를 채우는 것은 다른 종류의 일이다(로그는 그 런을 돌린 세션의 사실 기록이라, 사후에 보고서에서 옮겨 적는 것과 그때 적는 것이 같지 않다). **적을 때 그 출처가 보고서였다는 것을 함께 적는다.**

39. **게이트 ⑤(01~04 실물 완주)가 템플릿 위에서 한 번도 안 돌았다.** [[ADR-H013]] 의 다섯 줄 중 넷은 정적 검사이고 다섯째만 실물이며, 그 다섯째를 *"정적 검사만으로 통과시키면 「검증한 것만 승격한다」가 무너진다"* 는 이유로 남겨 뒀다. 세션 G 는 사람이 정한 범위(**기계 검사만**)에 따라 ⑤를 **안 돌렸고 미실행으로 기록했다** ([[ADR-H040]] 결정 2). 파일럿에서는 2026-09-03 에 통과했으므로 (P1) 이 빈칸이 가리키는 것은 *"하네스가 도는가"* 가 아니라 ***"클론한 템플릿이 첫 런을 완주시키는가"*** 다 — `calibration.json` 이 `state: "unmeasured"` 이고 어댑터가 `verified: false` 인 상태에서 01~04 가 어디서 멈추는지는 아무도 잰 적이 없다. **템플릿으로 프로젝트를 실제로 시작할 때 닫는다.** 두 리포는 독립이므로(36번) 이 항목의 실행 자리는 이 리포가 아니다.

40. **줄 예산 추정이 리뷰 라운드가 요구할 테스트를 세지 않는다.** P8 의 플랜 §5 가 **370~495줄**을 추정했고 실측은 **846줄**(문서 제외)이었다 — **초과분의 대부분(453줄)이 테스트**다. 01 이 아홉 라운드를 돌며 케이스를 하나씩 요구한 결과이므로, **「리뷰 라운드 수와 테스트 줄 수가 같이 자란다」**가 P8 이 처음 보여 준 관계다. P8 보고서가 「다음 런에서 바꿀 것」 ③으로 적었으나 **M 번호도 이 목록의 자리도 받지 못했고**, 그래서 위 38번을 닫으며 여기 등록한다 (2026-09-10).

    **표본이 하나다 — 값을 지금 정하지 않는다.** `precheck` 의 `files_max: 10` · `lines_max: 400` 은 미검증 상속값이고 **네 런 연속 초과**(P5·P6·P7·P8)인데, 그 넷 중 라운드 수와의 관계가 보이는 것은 P8 하나뿐이다. **잰 적 없는 축 위에서 상수를 다시 맞추는 것**은 [[ADR-H007]]·[[ADR-H026]] 이 경고한 모양이므로, **P9~P11 이 라운드 수와 실측 줄 수를 함께 남긴 뒤에 본다.** 그때까지 이 항목이 요구하는 것은 값이 아니라 **「추정과 실측을 런마다 한 자리에 남기는 것」**이다.
...
### C7 — M58·M59 를 닫는다 (2026-09-10 닫힘)

세션 D(추출)의 **첫 결정**이었다. `ROADMAP` §6 항목 36 이 *"열린 결함을 안은 채
템플릿으로 굳히지 않는다"* 를 D 의 선행 조건으로 걸어 두었고, 둘 다 **수정 범위가
좁아 안고 가는 값이 없어서** 닫고 갔다.

| ID | 무엇을 고쳤나 | 커밋 |
|---|---|---|
| **M58** | `counter_grant` 의 노드 초기값을 `{"max": extra}` → `{"max": 0}`. `max` 는 `grants` 의 파생값이므로 소모 없는 상태의 상한은 **지급 합계**와 같아야 한다 | `4febf81` |
| **M59** | 세션 원장이 **만진 런과 안 만진 런을 가른다.** 겹치면 `basis: touched`, 안 겹치면 `run_id` + `latest_only` 만 | `a860d09` · [[ADR-H036]] |

**M58 은 실물 경로로 도달하지 않는 결함이었다** — 지급은 `_grant_rounds` 한 곳에서
`round` 에만 일어나고 01 이 그 전에 반드시 라운드를 소모해 노드가 이미 있다. 재현이
라이브러리 직접 호출뿐이었고, **드러나지 않는다는 것이 안전하다는 뜻은 아니다**
(M11 이 다섯 런 동안 같은 자리를 지났다).

**M59 는 실물에서 이미 세 줄을 틀렸다.** P8 은 2026-09-09 01:08 에 닫혔는데
`09:05`·`10:21`(M56 수정)·`09-10 13:12`(C5) 세션이 전부 `20260908-1720-dca1` 을
달았고 뒤 둘은 그 런을 만진 적이 없다. **`tests` 칸도 같이 틀렸다** — 마지막 줄이
그 런의 `1459` 를 실었는데 C5 의 실제 앱 테스트는 **1,460** 이다.

**침묵으로 지우지 않은 것이 이 증분의 결정이다.** 가장 작은 수정은 `run` 칸을 아예
안 만드는 것이었는데, 그러면 *"런은 있었는데 이 세션이 안 만졌다"* 와 *"파이프라인
런이 아예 없던 세션"* 이 원장에서 같은 모양이 된다. 그리고 **판정 함수를 하나로
합쳤다** — `state.session_touched_run` 을 쓰는 쪽(`session_log`)과 읽는
쪽(`cost-state`)이 같이 부른다. 갈라지면 원장에 적힌 `basis` 와 그 줄을 읽어 낸
판정이 달라지고, 그것이 곧 원장이 자기와 모순되는 것이다.

**원장은 한 비트도 안 바뀌었다.** 소급 부여를 안 했고, 실물 원장 51줄에 대한
`cost-state` 판정을 수정 전후로 덤프해 대조했더니 diff 가 비었다 — C3·C4·C5 와
같은 방법이다.

**되돌려 무는 것을 확인했다.** M58 은 `test_소모_전_첫_지급이_상한을_두_배로_만들지
_않는다`, M59 는 `TestSessionLedger` 넷과 `TestRunCost` 의 일치 검사가 문다.

### D — 이력 추출 + 리포 골격 (2026-09-10 닫힘)

**추출 방침과 코드가 정반대를 말하고 있었다.** 항목 36 은 *"순차 실행기는 뺀다"* 고
적는데 **8페이즈 코어가 그 실행기를 import 하고 있었다** — `state.py`·`cli.py`·
`session_log.py` 셋이다. `state.py` 의 것은 모듈 최상위라, 그대로 추출했으면
`test_pipeline.py` **712 건이 수집 단계에서 전멸**하고 D 는 *"됐다"* 고 말할 게이트가
하나도 없었다. **A 가 `NODE_RUNNERS` 에서 만난 것과 같은 층의 문제인데 자물쇠가
없었다.**

`scripts/runtime.py` 가 공유 원시요소 다섯(`TZ` · `TRANSCRIPT_ROOT` ·
`force_utf8_output` · `read_cost_state` · `read_session_metrics`)을 들고, 코어와
실행기가 **둘 다 그것을 읽는다.** 옮긴 것 중 `subprocess` 를 부르는 조각은 없다 —
승인 우회는 실행기에 남는다 ([[ADR-H037]]). `state.RunningFile` 재수출은 소비자가
0 이라 옮기지 않고 지웠다.

**자물쇠가 회귀를 든다.** `CoreDoesNotImportTheExecutorTest` 가
`CoreHasNoStackNamesTest` 옆에 앉는다. 되돌려 무는 것을 확인했다.

**예측을 추출 *전에* 적었고 셋 다 그대로 나왔다.** 추출본은 815 통과 · 3 실패 ·
2 skip 이고 실패 셋은 미리 적은 것과 같다(어댑터 `requires` 둘 → E, 리모트 부재
하나 → G). **넷째는 없었다.**

**한 번은 예측이 틀렸고 그것이 이 절차의 값이다.** *"이력 전체에
`dangerously-skip-permissions` 가 0건"* 이라고 적었는데 66건이 나왔다 — 전부 이 문서와
`DECISIONS.md` 가 **왜 그것을 안 싣는가**를 설명하는 산문이고, 지우면 안 되는
것들이다. 검사가 코드와 그것을 설명하는 산문을 같은 것으로 셌다. 실행 가능한 파일로
좁혀 다시 재니 **0건**이다 — 방침은 지켜졌고 틀린 것은 재는 자였다. 예측을 먼저
적었기 때문에 이 구분이 사후 합리화가 아니라 채점으로 남는다.

**실측**: 커밋 362 → **187** · 파일 **69** · 테스트 **820** · 수집 에러 **0** ·
`lint-phases` exit 0. shelfie-app 쪽은 파이썬 **975 → 1,005** · 앱 **1,460** ·
`doctor` 0 · `src/` 변경 **0줄**.

**추출본에 커밋을 하나도 만들지 않았다.** 골격은 `filter-repo` 가 만든 트리 그
자체이고, 손으로 얹는 것은 E·F 의 몫이다. **리모트도 없다** — push 는 G 에서 한 번뿐이다.

파이썬 테스트 **967 → 975** · 앱 1,460 · `doctor` 0 · `lint-phases` 0 ·
`src/` 변경 0줄.

**D 의 진입을 막는 것이 더는 없다.** 어댑터 `verified: true`(위 14번)는 기다리지
않기로 이미 결정돼 있고, 템플릿은 `false` 를 그대로 싣고 이유를 문서에 적는다.
