...
**셋째 갈림이 이미 유력하다는 것을 숨기지 않는다.** `distinct_keys ≈ count` 라 **임계를 낮춰도 승격은 안 열린다.** 그런데도 축을 지금 안 바꾸는 이유는 ADR-H026 의 이유 ①이 안 풀렸기 때문이다 — 관측의 다수가 `target_role: main` 이라 **승격의 산물이 갈 곳조차 없다.** 결론을 미리 적는 것과 결론대로 지금 고치는 것은 다르다.
...
파이썬 테스트 **876 → 884** · `doctor` 0 · `lint-phases` 0 · `src/` 변경 0줄.
