...
테스트는 배선을 보고 런은 비용을 본다.

| 무엇 | 어떻게 확인되는가 |
|---|---|
| M37·M38 | 05 제출이 한 번에 통과하는가 — `attempts` 가 비어 있으면 봉투가 맞았다 |
| M40 | `precheck.at_06` 이 PR 본문의 diff 통계와 같은 파일 수를 말하는가 |
| M40 부수 | 06 에서 예산 exit 9 가 나는가. 나면 사람 판단이고, 그 판단이 `budget` 상수의 첫 실측이 된다 |
| M43 | 델타 라운드 뒤 보고서의 "05 리뷰어" 칸이 1회차 수를 적는가 |
| M44 | 키 없이 05 를 지나며 `infra_skipped:` 가 등급을 내리며 남는가 |
| M39 | P6 원장에 `DOC_CODE_DRIFT` 가 실제로 쓰이는가. 0건이면 어휘가 아니라 리뷰어 프롬프트 문제다 |
| **M36** | **아직 안 돌았다** — 상한을 실제로 넘는 런이 없으면 배선만 확인된 것이다. **통과로 적지 않는다** |

**여전히 자연 실패를 기다리는 셋**(M32·M33·M35)과 어댑터 `verified` 는 이 증분이 건드리지 않았다. 일부러 만들지 않는다.
