...
| `advance --phase` | 명시 전이(수동 개입·복구용). 산출물 신선도 + **워크트리 지문 대조** | 0 / 6 |
...
| `report --out <p>` | 결정론 표 조립 + 필수 섹션 검사 | 0 / 6 |
...
1. **워크트리 지문 결합** — 게이트 통과 후 소스가 바뀌면 영수증이 stale(exit 6). "통과한 셈 치고 넘어가기"의 구조적 차단.
...
- 같은 `run_id`로 재개해 다시 쓰면 **덮어쓴다**(최종본이 맞다).
