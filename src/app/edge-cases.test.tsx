 ...
 * | 5 | 부분 실패 | 일부 잠김 · **결함 보고** | 배너와 분모는 `booklist/BookList.test.tsx` "목록보다 앞에 렌더되고 분모를 함께 밝힌다", 실패분만 다시 보내는 것은 `app/page.test.tsx` "실패한 사진만 재시도하면 …"(확인 0건 화면). **확인된 책이 1권 이상인 `reviewing` 화면에서는 재시도가 먹지 않는다** — 리듀서가 `reviewing`에서 온 `ANALYZE_RETRIED`를 처리하지 않아 상태가 그대로 남고, 그러면 뒤이어 오는 `ANALYZE_SUCCEEDED`도 `status !== "analyzing"`으로 버려진다. 호출은 나갔으므로 **모델 비용만 들고 화면은 그대로다.** `lib/session.ts` 수정은 이 step의 금지 범위라 고치지 않았다 |
