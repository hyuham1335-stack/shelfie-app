...
/* ------------------------------------------------------------------ *
 * 레이트 리밋 (TR-013 · TRD 7번 · ADR-012)
 *
 * 다섯 개 전부 여기 리터럴로 둔다. `process.env`를 읽지 않는다 — 이 모듈은
 * 클라이언트에서도 import되고, 로드 시점에 던지지 않는 것이 이 파일의 규율이다.
 * 판정 코드(`lib/rate-limit.ts`)와 라우트, 테스트 어디에도 같은 값의 리터럴을
 * 두지 않는다. 두 벌이 되면 한쪽만 고쳐지는 날이 온다.
 * ------------------------------------------------------------------ */

/**
 * IP 하나가 한 윈도에 낼 수 있는 요청 수 (TRD 7번 "IP당 분당 20회").
 *
 * 이 값은 **한 인스턴스 안에서만** 유효하다. 공유 스토어가 없으므로 실효 상한은
 * 인스턴스 수에 비례하고, 그 수는 재지 않았다 (ADR-012).
 */
export const RATE_LIMIT_MAX_REQUESTS = 20;

/** 고정 윈도의 길이. "분당"이 곧 이 값이다 */
export const RATE_LIMIT_WINDOW_MS = 60_000;

/**
 * 한 윈도에 추적하는 키 수의 상한. 넘으면 신규 키는 공유 버킷으로 간다.
 *
 * **이 값은 재지 않았다.** 인메모리 맵이 무한히 자라지 않게 하려고 둔 것이고,
 * 이 서비스의 실제 IP 분포나 함수 메모리 여유를 측정해서 고른 수가 아니다.
 * 근거를 적을 수 없으므로 근거 대신 그 사실을 적는다.
 */
export const RATE_LIMIT_MAX_TRACKED_KEYS = 50_000;

/**
 * 공유 버킷(`__shared:unknown__`·`__shared:overflow__`)의 상한.
 *
 * 공유 버킷은 여러 클라이언트를 함께 담는 자리라 한 사람 몫(20회)을 쓰면 인프라
 * 결함 하나로 전원이 즉시 막힌다. 그래서 상한을 분리한다.
 * **다만 이 값(200)도 재지 않았다** — 20의 열 배라는 것 말고 근거가 없다.
 */
export const RATE_LIMIT_SHARED_MAX_REQUESTS = 200;

/**
 * 레이트 리밋 키에 담는 주소 문자열의 최대 길이.
 *
 * 위의 넷과 달리 이 값은 **규격에서 온다** — IPv4 사상 IPv6 최대 표기
 * (`0000:0000:0000:0000:0000:ffff:255.255.255.255`)가 45자다. 재고 말고가 없다.
 * 키 전체의 길이 48은 여기에 `"ip:".length`를 더한 **유도값**이라 상수로 두지 않는다.
 */
export const RATE_LIMIT_MAX_KEY_CHARS = 45;
