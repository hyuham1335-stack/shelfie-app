 ...
 * 여기서 다시 구현하면 판정 기준이 두 벌이 되어 소리 없이 갈린다. 그래서 이
 * 모듈의 출력은 `judge()`가 그대로 받아먹는 `LookupOutcome` 하나뿐이다.
 *
...
import { aladinCandidateSchema } from "@/lib/schemas";
...
export async function searchByTitle(
  ...
  const url = buildSearchUrl(ttbKey, title, author);

  let attempt = await callOnce(url, remainingTimeout(deadlineAt), fetchImpl);

  // 재시도는 5xx·타임아웃·네트워크 오류에만 한다. 4xx는 우리 요청이 잘못된
  // 것이므로 반복해도 같고, 호출 한 번은 일일 5,000회 한도에서 그대로 빠진다.
  if (attempt.kind === "retryable") {
    const remaining = remainingTimeout(deadlineAt);
    if (remaining > 0) {
      attempt = await callOnce(url, remaining, fetchImpl);
    }
  }

  if (attempt.kind === "ok") {
    breaker?.recordSuccess();
    return { status: "ok", candidates: attempt.candidates };
  }
...
type Attempt =
  | { kind: "ok"; candidates: AladinCandidate[] }
  /** 다시 걸면 달라질 수 있는 실패 — 5xx·타임아웃·네트워크 오류 */
  | { kind: "retryable" }
  /** 다시 걸어도 같은 실패 — 4xx·응답 형태 어긋남 */
  | { kind: "fatal" };

async function callOnce(url: string, timeoutMs: number, fetchImpl: typeof fetch): Promise<Attempt> {
 ...
 */
function parseSearchResponse(body: string): Attempt {
  let raw: unknown;
  try {
    raw = JSON.parse(body);
  } catch {
  ...
  if (items === undefined) {
    ...
    if (envelope.data.totalResults === 0) return { kind: "ok", candidates: [] };
